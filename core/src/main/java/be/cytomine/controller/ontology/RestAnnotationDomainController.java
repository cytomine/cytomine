...
public class RestAnnotationDomainController extends RestCytomineController {
    ...
    public void download(@PathVariable Long project,
                         @RequestBody AnnotationReportParams params) throws IOException {
        ...
        String format = (params.format() == null || params.format()
            .isBlank()) ? "pdf" : params.format();
    ...
    public ResponseEntity<byte[]> crop(@PathVariable Long id,
                                       ...
                                       ProxyExchange<byte[]> proxy) throws IOException, ParseException {
    ...
    public ResponseEntity<String> listIncludedAnnotation(@PathVariable(name = "image") Long imageId) throws IOException {
        JsonObject jsonObject = mergeQueryParamsAndBodyParams();
        jsonObject.put("image", imageId);
        return responseSuccess(getIncludedAnnotation(jsonObject, paramsService.getPropertyGroupToShow(jsonObject)));
    }
    ...
    private List getIncludedAnnotation(JsonObject params, List<String> propertiesToShow) {
        ...
        if (geometry == null) {
            ...
            geometry = annotation.getLocation()
                .toText();
        }
        ...
        User user = null;
        if (idUser != 0) {
            user = userService.find(params.getJSONAttrLong("user"))
                .orElse(null);
        }
    ...
    public ResponseEntity<String> add(@RequestBody String json,
                                      ...
                                      @RequestParam(required = false) Long maxPoint) throws IOException {
    ...
    public ResponseEntity<String> simplify(@PathVariable Long id,
                                           @RequestParam(required = false) Long minPoint,
                                           @RequestParam(required = false) Long maxPoint

    ) {
        AnnotationDomain annotation = AnnotationDomain.getAnnotationDomain(entityManager, id);
        SimplifiedAnnotation simplifiedAnnotation = simplifyGeometryService.simplifyPolygon(annotation.getLocation(),
            minPoint,
            maxPoint);
    ...
    public ResponseEntity<String> retrieveSimplify(@RequestBody JsonObject jsonObject,
                                                   @RequestParam(required = false) Long minPoint,
                                                   @RequestParam(required = false) Long maxPoint

    ) {
        SimplifiedAnnotation simplifiedAnnotation = simplifyGeometryService.simplifyPolygon(jsonObject.getJSONAttrStr(
            "wkt"), minPoint, maxPoint);
        return responseSuccess(JsonObject.of("wkt",
            simplifiedAnnotation.getNewAnnotation()
                .toText()));
    }
    ...
    public ResponseEntity<String> fillAnnotation(@PathVariable Long id) throws IOException {
        AnnotationDomain annotation = AnnotationDomain.getAnnotationDomain(entityManager, id);

        //Is the first polygon always the big 'boundary' polygon?
        String newGeom = GeometryUtils.fillPolygon(annotation.getLocation()
            .toText());
    ...
    public ResponseEntity<String> addCorrection(@RequestBody JsonObject jsonObject) throws ParseException {
        ...
        if (idsUserAnnotation.isEmpty()) {
            return responseSuccess(reviewedAnnotationService.doCorrectReviewedAnnotation(idsReviewedAnnotation,
                location,
                remove));
        } else {
    ...
    public ResponseEntity<String> processAnnotationWithSam(@PathVariable Long id) throws UnsupportedEncodingException, ParseException {
        ...
        cropParameter.setLocation(annotation.getLocation()
            .toString());
        ...
        body.add("image_width",
            annotation.getImage()
                .getBaseImage()
                .getWidth());
        body.add("image_height",
            annotation.getImage()
                .getBaseImage()
                .getHeight());
        body.add("location",
        ...
        URI url = UriComponentsBuilder.fromUriString(this.samUrl)
