...
def validate_box_feature(feature: Dict[str, Any]) -> None:
    """
    Validate a box feature, ensure that the feature corresponds to a box.
    ...
    if len(box) != 5:
        raise HTTPException(
            400,
            "Box must have 5 coordinates (4 corners + closing point).",
        )

    if box[0] != box[-1]:
        raise HTTPException(
            400,
            "Box must be closed (first and last coordinates must match).",
        )
...
def validate_point_feature(feature: Dict[str, Any]) -> None:
    """
    Validate a point feature, ensure that the feature corresponds to a point.
    ...
    if "label" not in properties or properties["label"] not in (0, 1):
        raise HTTPException(
            400,
            "Point must have a 'label' property with value 0 or 1.",
        )
