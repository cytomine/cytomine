...
public class UserAnnotationResourceTests {
    ...
    public void get_a_user_annotation() throws Exception {
        ...
        restUserAnnotationControllerMockMvc.perform(get("/api/userannotation/{id}.json",
    ...
    public void list_annotations_light() throws Exception {
        UserAnnotation userAnnotation = builder.given_a_user_annotation();
        restUserAnnotationControllerMockMvc.perform(get("/api/userannotation.json",
                userAnnotation.getId()))
            .andExpect(status().isOk())
    ...
    public void count_annotations_by_user() throws Exception {
        UserAnnotation userAnnotation = builder.given_a_user_annotation();
        restUserAnnotationControllerMockMvc.perform(get("/api/user/{idUser}/userannotation/count" +
                ".json", userAnnotation.getUser().getId()))
            .andExpect(status().isOk())
    ...
    public void count_annotations_by_project_with_dates() throws Exception {
        ...
        restUserAnnotationControllerMockMvc.perform(get("/api/project/{idProject}/userannotation" +
                "/count.json", oldUserAnnotation.getProject().getId())
        ...
        restUserAnnotationControllerMockMvc.perform(get("/api/project/{idProject}/userannotation" +
                "/count.json", oldUserAnnotation.getProject().getId())
                .param("startDate",
                    String.valueOf(DateUtils.addSeconds(oldUserAnnotation.getCreated(), -1).getTime())))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.total").value(2));
        ...
        restUserAnnotationControllerMockMvc.perform(get("/api/project/{idProject}/userannotation" +
                "/count.json", oldUserAnnotation.getProject().getId())
                .param("endDate",
                    String.valueOf(DateUtils.addSeconds(newUserAnnotation.getCreated(), 1).getTime())))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.total").value(2));
        ...
        restUserAnnotationControllerMockMvc.perform(get("/api/project/{idProject}/userannotation" +
                "/count.json", oldUserAnnotation.getProject().getId())
                .param("endDate", String.valueOf(DateUtils.addDays(oldUserAnnotation.getCreated()
                    , -2).getTime())))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.total").value(0));
    }
    ...
    public void download_user_annotation_pdf_document() throws Exception {
        buildDownloadContext();
        restAnnotationDomainControllerMockMvc.perform(get("/api/project/{project}/userannotation" +
                "/download", this.project.getId())
                .param("format", "pdf")
    ...
    private void buildDownloadContext() throws ParseException {
        ...
        this.userAnnotation = builder.given_a_user_annotation(this.slice, "POLYGON((1 1,5 1,5 5,1" +
            " 5,1 1))", this.me, this.term);
    }

    private void checkResult(String delimiter, MvcResult result) throws UnsupportedEncodingException {
        TestUtils.checkSpreadsheetAnnotationResult(delimiter, result, this.userAnnotation,
            this.project, this.image, this.me, this.term, "userannotation",
            applicationProperties.getServerURL());
    }

    private void checkXLSResult(MvcResult result) throws IOException {
        TestUtils.checkSpreadsheetXLSAnnotationResult(result, this.userAnnotation, this.project,
            this.image, this.me, this.term, "userannotation", applicationProperties.getServerURL());
    }
    ...
    private MvcResult performDownload(String format) throws Exception {
        return restAnnotationDomainControllerMockMvc.perform(get("/api/project/{project" +
                "}/userannotation/download", this.project.getId())
                .param("format", format)
                .param("users", this.me.getId().toString())
                .param("reviewed", "false")
                .param("terms", this.term.getId().toString())
    ...
    public void add_user_annotation_with_not_valid_location() throws Exception {
        ...
        jsonObject.put("location",
            "POLYGON ((225.73582220103702 306.89723126347087, 225.73582220103702 307" +
                ".93556995227914, 226.08028300710947 307.93556995227914, 226.08028300710947 306" +
                ".89723126347087, 225.73582220103702 306.89723126347087))"
        ); // too small
        restUserAnnotationControllerMockMvc.perform(post("/api/userannotation.json")
                .contentType(MediaType.APPLICATION_JSON)
    ...
    public void edit_valid_user_annotation() throws Exception {
        UserAnnotation userAnnotation = builder.given_a_user_annotation();
        restUserAnnotationControllerMockMvc.perform(put("/api/userannotation/{id}.json",
                userAnnotation.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(userAnnotation.toJSON()))
            .andExpect(status().isOk())
    ...
    public void delete_user_annotation() throws Exception {
        ...
        restUserAnnotationControllerMockMvc.perform(delete("/api/userannotation/{id}.json",
                userAnnotation.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(userAnnotation.toJSON()))
            .andExpect(status().isOk())
    ...
    public void get_user_annotation_crop() throws Exception {
        ...
        MvcResult mvcResult = restUserAnnotationControllerMockMvc.perform(get("/api" +
                "/userannotation/{id}/crop.png?maxSize=512", annotation.getId()))
            .andExpect(status().isOk())
            .andReturn();
    ...
    public void get_user_annotation_crop_mask() throws Exception {
        ...
        String body = "{\"length\":512,\"z_slices\":0,\"annotations\":[{\"geometry\":\"POLYGON (" +
            "(1 1, 50 10, 50 50, 10 50, 1 1))\",\"fill_color\":\"#fff\"}],\"timepoints\":0}";
        System.out.println(url);
        System.out.println(body);
        stubFor(post(urlEqualTo(IMS_API_BASE_PATH + url)).withRequestBody(equalTo(
                    body
        ...
        MvcResult mvcResult = restUserAnnotationControllerMockMvc.perform(get("/api" +
                "/userannotation/{id}/mask.png?maxSize=512", annotation.getId()))
            .andExpect(status().isOk())
            .andReturn();
        List<LoggedRequest> all = wireMockServer.findAll(RequestPatternBuilder.allRequests());
        AssertionsForClassTypes.assertThat(mvcResult.getResponse().getContentAsByteArray()).isEqualTo(mockResponse);
    ...
    public void get_user_annotation_alpha_mask() throws Exception {
        ...
        MvcResult mvcResult = restUserAnnotationControllerMockMvc.perform(get("/api" +
                "/userannotation/{id}/alphamask.png", annotation.getId()))
            .andExpect(status().isOk())
            .andReturn();
        List<LoggedRequest> all = wireMockServer.findAll(RequestPatternBuilder.allRequests());
        AssertionsForClassTypes.assertThat(mvcResult.getResponse().getContentAsByteArray()).isEqualTo(mockResponse);
    ...
    public void create_comments_for_annotation() throws Exception {
        ...
        restUserAnnotationControllerMockMvc.perform(post("/api/userannotation/{id}/comment.json",
    ...
    public void get_comment_for_annotation() throws Exception {
        ...
        restUserAnnotationControllerMockMvc.perform(get("/api/userannotation/{annotation}/comment" +
                "/{id}.json", userAnnotation.getId(), comment.getId()))
            .andExpect(status().isOk())
