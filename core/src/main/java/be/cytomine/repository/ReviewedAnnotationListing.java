...
public class ReviewedAnnotationListing extends AnnotationListing {
    ...
    LinkedHashMap<String, AvailableColumns> getAvailableColumn() {
        ...
        term.put("userTerm", "a.user_id"); //user who add the term, is the user that create
        // reviewedannotation (a.user_id)
        map.put("term", term);
    ...
    String getFrom() {
        ...
        if (multipleTerm) {
            from += "LEFT OUTER JOIN reviewed_annotation_term at ON a.id = at" +
                ".reviewed_annotation_terms_id ";
            from += "LEFT OUTER JOIN reviewed_annotation_term at2 ON a.id = at2" +
                ".reviewed_annotation_terms_id ";
            ...
            from = from + " LEFT OUTER JOIN reviewed_annotation_term at ON a.id = at" +
                ".reviewed_annotation_terms_id ";
    ...
    String buildExtraRequest() {
        ...
        if (kmeansValue == 3 && image != null && bbox != null) {
            /**
             * We will sort annotation so that big annotation that covers a lot of annotation
             * comes first (appear behind little annotation so we can select annotation behind
             * other)
             * We compute in 'gc' the set of all other annotation that must be list
             * For each review annotation, we compute the number of other annotation that cover
             * it (ST_CoveredBy => t or f => 0 or 1)
             *
             * ST_CoveredBy will return false if the annotation is not perfectly "under" the
             * compare annotation (if some points are outside)
             * So in gc, we increase the size of each compare annotation just for the check
             * So if an annotation x is under y but x has some point next outside y, x will
             * appear top (if no resize, it will appear top or behind).
             */
            String xfactor = "1.28";
            String yfactor = "1.28";
            ImageInstance imageInstance = entityManager.find(ImageInstance.class, image);
            //TODO:: get zoom info from UI client, display with scaling only with hight zoom (<
            // annotations)
            ...
            if (zoomToLow) {
                ...
                subRequest = "(SELECT SUM(ST_CoveredBy(ga.location,ST_Translate(ST_Scale(gb" +
                    ".location, " + xfactor + "," + yfactor + "), ST_X(ST_Centroid(gb.location))*" +
                    "(1 - " + xfactor + "), ST_Y(ST_Centroid(gb.location))*(1 - " + yfactor + ") " +
                    "))::integer) ";
