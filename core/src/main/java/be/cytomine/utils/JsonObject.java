...
public class JsonObject extends HashMap<String, Object> implements JsonInput {
    ...
    public static JsonObject of(String key, Object value, String key2, Object value2, String key3
        , Object value3) {
    ...
    public static JsonObject of(String key, Object value, String key2, Object value2, String key3
        , Object value3, String key4, Object value4, String key5, Object value5, String key6,
                                Object value6) {
        JsonObject jsonObject = new JsonObject();
        jsonObject.put(key, value);
        jsonObject.put(key2, value2);
     ...
     */
    public CytomineDomain getJSONAttrDomain(EntityManager entityManager, String attr,
                                            CytomineDomain domain, boolean mandatory) {
        return getJSONAttrDomain(entityManager, attr, domain, "id", "Long", mandatory);
    }

    /**
     * Get attr domain value from json
     * Read domain thanks to domain argument, get the correct object thanks to value from column
     * (type: columnType)
     * If mandatory flag is true, check if domain exists
     *
     * @return Value as Cytomine Domain
     */
    public CytomineDomain getJSONAttrDomain(EntityManager entityManager, String attr,
                                            CytomineDomain domain, String column,
                                            String columnType, boolean mandatory) {
        if (this.get(attr) != null && !this.get(attr).toString().equals("null")) {
            ...
            if (this.get(attr) instanceof CytomineDomain) {
                ...
                domainRead = entityManager.find(domain.getClass(),
                    Long.parseLong(this.get(attr).toString()));
            } else {
                String request =
                    "SELECT d FROM " + domain.getClass().getName() + " d WHERE " + column + " = " +
                        ":value";
                ...
                if (domainRead == null) {
                    throw new WrongArgumentException(domain.getClass().getName() + " cannot be " +
                        "found with " + column + " = '" + convertValue(this.get(attr),
                        columnType) + "'");
                }
            }
            if (domainRead == null) {
