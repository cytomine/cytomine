...
public class BootstrapDataService {
    ...
    public void initData() {
        ...
        bootstrapUtilsService.createUser("ImageServer1", "Image", "Server", List.of("ROLE_USER",
            "ROLE_ADMIN", "ROLE_SUPER_ADMIN"));

        bootstrapUtilsService.createRelation(PARENT);

        bootstrapUtilsService.createConfigurations("WELCOME", "<p>Welcome to the Cytomine " +
            "software.</p><p>This software is supported by the <a href='https://cytomine" +
            ".coop'>Cytomine company</a></p>", ConfigurationReadingRole.ALL);
        bootstrapUtilsService.createConfigurations("admin_email",
            applicationProperties.getAdminEmail(), ConfigurationReadingRole.ADMIN);
    }
    ...
    public void initImageFilters() {
        ...
        List<Map<String, Object>> filters = List.of(
            ...
            Map.of("name", "Haematoxylin", "method", "haematoxylin", "available", false), //To be
            // removed: does not exist
            Map.of("name", "Eosin", "method", "eosin", "available", false), //To be removed: does
            // not exist
        ...
        for (Map<String, Object> filter : filters) {
            bootstrapUtilsService.createFilter((String) filter.get("name"), (String) filter.get(
                "method"), (Boolean) filter.get("available"));
        }
    }
    ...
    private void changeUserKeys(String username, String privateKey, String publicKey) {
        User user = userRepository.findByUsernameLikeIgnoreCase(username)
            .orElseThrow(() -> new ObjectNotFoundException(username + " user does not exists, " +
                "cannot set its keys"));
    ...
    public void updateImageFilters() {
        ImageFilter imageFilter =
            imageFilterRepository.findAll().stream().filter(x -> x.getName().equals("Binary")).findFirst().orElse(null);
        if (imageFilter != null) {
