...
public class ImageFilterProjectService extends ModelService {
    ...
    public CommandResponse add(JsonObject jsonObject) {
        ...
        Project project = projectRepository.findById(jsonObject.getJSONAttrLong("project"))
            .orElseThrow(() -> new ObjectNotFoundException("Project", jsonObject.getJSONAttrStr(
                "project")));
        ImageFilter imageFilter = imageFilterRepository.findById(jsonObject.getJSONAttrLong(
                "imageFilter"))
            .orElseThrow(() -> new ObjectNotFoundException("ImageFilter",
                jsonObject.getJSONAttrStr("imageFilter")));
    ...
    public List<Object> getStringParamsI18n(CytomineDomain domain) {
        ...
        return List.of(imageFilterProject.getImageFilter().getName(),
    ...
    public CytomineDomain retrieve(JsonObject json) {
        if (json.containsKey("id")) {
            return imageFilterProjectRepository.findById(json.getId())
                .orElseThrow(() -> new ObjectNotFoundException("ImageFilterProject",
                    json.toJsonString()));
        }
