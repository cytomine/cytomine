...
public class SliceInstanceResourceTests {
    ...
    public void list_slice_instance_by_image_instance() throws Exception {
        ...
        restSliceInstanceControllerMockMvc.perform(get("/api/imageinstance/{id}/sliceinstance" +
                ".json", sliceInstance.getImage().getId()))
    ...
    public void get_an_slice_instance() throws Exception {
        ...
        restSliceInstanceControllerMockMvc.perform(get("/api/sliceinstance/{id}.json",
                image.getId()))
            .andExpect(status().isOk())
    ...
    public void get_an_slice_instance_with_coordinates() throws Exception {
        ...
        restSliceInstanceControllerMockMvc.perform(get("/api/imageinstance/{id}/{channel}/{zStack" +
                    "}/{time}/sliceinstance.json",
                image.getImage().getId(), image.getBaseSlice().getChannel(),
                image.getBaseSlice().getZStack(), image.getBaseSlice().getTime()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.id").value(image.getId().intValue()));
    }
    ...
    public void edit_valid_slice_instance() throws Exception {
        ...
        restSliceInstanceControllerMockMvc.perform(put("/api/sliceinstance/{id}.json",
                sliceInstance.getId())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    public void delete_slice_instance() throws Exception {
        SliceInstance sliceInstance = builder.given_a_slice_instance();
        restSliceInstanceControllerMockMvc.perform(delete("/api/sliceinstance/{id}.json",
                sliceInstance.getId()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.printMessage").value(true))
            .andExpect(jsonPath("$.callback").exists())
    ...
    public void get_slice_instance_thumb() throws Exception {
        ...
        MvcResult mvcResult = restSliceInstanceControllerMockMvc.perform(get("/api/sliceinstance" +
                "/{id}/thumb.png?maxSize=512", image.getId()))
            .andExpect(status().isOk())
            .andReturn();
    ...
    public void get_slice_instance_tile() throws Exception {
        ...
        configureFor("localhost", 8888);
        stubFor(get(urlEqualTo(IMS_API_BASE_PATH + "/image/" + URLEncoder.encode(image.getPath(),
            StandardCharsets.UTF_8).replace("%2F", "/") + "/normalized-tile/zoom/2/tx/4/ty/6" +
            "?z_slices=0&timepoints=0&filters=binary"))
            .willReturn(
                aResponse().withBody(mockResponse)
            )
        );
        ...
        MvcResult mvcResult = restSliceInstanceControllerMockMvc.perform(get("/api/sliceinstance" +
                    "/{id}/normalized-tile/zoom/{z}/tx/{tx}/ty/{ty}.jpg?filters=binary",
                image.getId(), 2, 4, 6))
            .andExpect(status().isOk())
            .andReturn();
        List<LoggedRequest> all = wireMockServer.findAll(RequestPatternBuilder.allRequests());
    ...
    public void get_slice_instance_tile_if_image_not_exist() throws Exception {
        restSliceInstanceControllerMockMvc.perform(get("/api/sliceinstance/{id}/normalized-tile" +
                "/zoom/{z}/tx/{tx}/ty/{ty}.jpg?filters=binary", 0, 2, 4, 6))
            .andExpect(status().isNotFound())
            .andExpect(jsonPath("$.errors").exists());
    }
    ...
    public void histograms() throws Exception {
        ...
        restSliceInstanceControllerMockMvc.perform(get("/api/sliceinstance/{id}/histogram.json",
                image.getId()))
            .andExpect(status().isOk())
    ...
    public void histograms_bounds() throws Exception {
        ...
        configureFor("localhost", 8888);
        stubFor(get(urlEqualTo(IMS_API_BASE_PATH + "/image/" + URLEncoder.encode(image.getPath(),
            StandardCharsets.UTF_8).replace("%2F", "/") + "/histogram/per-plane/z/0/t/0/bounds" +
            "?channels=0"))
            .willReturn(
                aResponse().withBody(
                    """
        ...
        restSliceInstanceControllerMockMvc.perform(get("/api/sliceinstance/{id}/histogram/bounds" +
                ".json", image.getId()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection", hasSize(equalTo(1))))
            .andExpect(jsonPath("$.collection[0].color").value("#f00"));
    ...
    public void channel_histograms() throws Exception {
        ...
        restSliceInstanceControllerMockMvc.perform(get("/api/sliceinstance/{id}/channelhistogram" +
                ".json", image.getId()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection", hasSize(equalTo(1))))
    ...
    private SliceInstance given_test_slice_instance() {
        ...
        SliceInstance sliceInstance =
            builder.given_a_slice_instance(builder.given_an_image_instance(image.getImage(),
                builder.given_a_project()), image);
