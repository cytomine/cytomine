...
public class ProjectRepresentativeUserService extends ModelService {
    ...
    public Optional<ProjectRepresentativeUser> find(Long id) {
        Optional<ProjectRepresentativeUser> optionalProjectRepresentativeUser =
    ...
    public CommandResponse delete(CytomineDomain domain, Transaction transaction, Task task,
        ...
        if (listByProject(((ProjectRepresentativeUser) domain).getProject()).size() < 2) {
            throw new WrongArgumentException("You cannot remove the last representative role. Add" +
                " someone else as representative");
        }
        User currentUser = currentUserService.getCurrentUser();
