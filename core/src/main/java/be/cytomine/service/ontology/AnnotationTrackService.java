...
public class AnnotationTrackService extends ModelService {
    ...
    public Optional<AnnotationTrack> find(AnnotationDomain annotation, Track track) {
        Optional<AnnotationTrack> optionalAnnotationTrack =
            annotationTrackRepository.findByAnnotationIdentAndTrack(annotation.getId(), track);
        optionalAnnotationTrack.ifPresent(AnnotationTrack -> securityACLService.check(AnnotationTrack.container(), READ));
        return optionalAnnotationTrack;
    }
    ...
    public CommandResponse add(JsonObject jsonObject) {
        AnnotationDomain annotation =
            annotationDomainRepository.findById(jsonObject.getJSONAttrLong("annotationIdent"))
                .orElseThrow(() -> new ObjectNotFoundException("Annotation",
                    jsonObject.getJSONAttrStr("annotationIdent")));
        securityACLService.check(annotation.container(), READ);
        ...
        return executeCommand(new AddCommand(currentUserService.getCurrentUser()), null,
            jsonObject);
    }

    public CommandResponse addAnnotationTrack(String annotationClassName, Long annotationIdent,
                                              Long idTrack, Long idSlice, Transaction transaction) {
    ...
    public CommandResponse delete(CytomineDomain domain, Transaction transaction, Task task,
        ...
        AnnotationDomain annotation =
            annotationDomainRepository.findById(annotationTrack.getAnnotationIdent())
                .orElseThrow(() -> new ObjectNotFoundException("Annotation",
                    annotationTrack.getId()));
    ...
    public List<String> getStringParamsI18n(CytomineDomain domain) {
        AnnotationTrack annotationTrack = (AnnotationTrack) domain;
        return Arrays.asList(String.valueOf(annotationTrack.getId()),
            String.valueOf(annotationTrack.getAnnotationIdent()),
            annotationTrack.getTrack().getName());
    }
    ...
    public void checkDoNotAlreadyExist(CytomineDomain domain) {
        ...
        if (annotationTrack != null) {
            if (annotationTrackRepository.findByAnnotationIdentAndTrack(annotationTrack.getId(),
                annotationTrack.getTrack()).stream().anyMatch(x -> !Objects.equals(x.getId(),
                annotationTrack.getId()))) {
            ...
            if (annotationTrackRepository.findBySliceAndTrack(annotationTrack.getSlice(),
                annotationTrack.getTrack()).stream().anyMatch(x -> !Objects.equals(x.getId(),
                annotationTrack.getId()))) {
    ...
    public CytomineDomain retrieve(JsonObject json) {
        ...
        if (json.containsKey("id") && !json.get("id").toString().equals("null")) {
            domain = (CytomineDomain) getEntityManager().find(currentDomain(),
                super.retrieveLongId(json)); // cast to long = issue ? TODO
            ...
            domain =
                annotationTrackRepository.findByAnnotationIdentAndTrack(json.getJSONAttrLong(
                        "annotationIdent"), track)
                    .orElse(null);
        }
