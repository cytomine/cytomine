...
public class AnnotationListingService {
    ...
    public List listGeneric(AnnotationListing al) {
        ...
        if ((al.getKmeans() != null && al.getKmeans()) && al.getKmeansValue() == null) {
            if (al.getBbox() == null) {
                throw new WrongArgumentException("If you want to use kmeans, you must provide image bbox:"
                    + al.getBbox());
            }
        ...
        for (Tuple rowResult : resultList) {
            ...
            if ((Long) tuple.get("id") != lastAnnotationId) {
                ...
                if (al.getColumnsToPrint().contains("track") && (al instanceof UserAnnotationListing)) {
                    ...
                    item.put(
                        "annotationTrack",
                        (tuple.get("track") != null ? buildList(new HashMap<>(Map.of(
                            "id",
                            tuple.get("annotationtracks"),
                            "track",
                            tuple.get("track")
                        ))) : new ArrayList<>())
                    );
                ...
                if (al.getColumnsToPrint().contains("group")) {
                    ...
                    item.put(
                        "annotationLink", (tuple.get("group") != null ? buildList(new HashMap<>(Map.of(
                ...
                if (al.getColumnsToPrint().contains("gis")) {
                    item.put(
                        "perimeterUnit",
                        tuple.get("perimeterunit") != null
                            ? GisUtils.retrieveUnit((Integer) tuple.get("perimeterunit"))
                            : null
                    );
                    item.put(
                        "areaUnit",
                        tuple.get("areaunit") != null ? GisUtils.retrieveUnit((Integer) tuple.get("areaunit")) : null
                    );
                ...
                if (termAsked && tuple.get("term") != null) {
                    ...
                    if ((Long) tuple.get("term") == lastTermId) {
                        ...
                        ((List) lastResult.get("userByTerm")).add(new HashMap<>(Map.of(
                            "id",
                            tuple.get("annotationterms"),
                            "term",
                            tuple.get("term"),
                ...
                if (trackAsked
                    && tuple.get("track") != null
                    && (long) tuple.get("track") != lastTrackId
                    && !((List) lastResult.get("track")).contains(tuple.get("track"))) {
                    ((List) lastResult.get("track")).add(tuple.get("track"));
                    ((List) lastResult.get("annotationTrack")).add(new HashMap<>(Map.of(
                        "id",
                        tuple.get("annotationtracks"),
                        "track",
                        tuple.get("track")
