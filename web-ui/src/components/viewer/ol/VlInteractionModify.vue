...
export default {
  ...
  created() {
    this.$interaction = new CustomModify({
      features: getIdent(this.source),
      deleteCondition: this.deleteCondition,
      insertVertexCondition: this.insertVertexCondition,
      pixelTolerance: this.pixelTolerance
    });
    this.$interaction.on('modifystart', evt => this.$emit('modifystart', evt));
    this.$interaction.on('modifyend', evt => this.$emit('modifyend', evt));
