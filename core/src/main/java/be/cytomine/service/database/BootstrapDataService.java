...
public class BootstrapDataService {
    ...
    public void initData() {
        ...
        bootstrapUtilsService.createUser(
            "ImageServer1",
            "Image",
            "Server",
        ...
        bootstrapUtilsService.createConfigurations(
            "admin_email",
            applicationProperties.getAdminEmail(),
            ConfigurationReadingRole.ADMIN
        );
    }
    ...
    public void initImageFilters() {
        ...
        for (Map<String, Object> filter : filters) {
            bootstrapUtilsService.createFilter(
                (String) filter.get("name"),
                (String) filter.get("method"),
                (Boolean) filter.get("available")
            );
        }
    }

    public void updateImageFilters() {
        ImageFilter imageFilter = imageFilterRepository.findAll()
            .stream()
            .filter(x -> x.getName().equals("Binary"))
            .findFirst()
            .orElse(null);
        if (imageFilter != null) {
