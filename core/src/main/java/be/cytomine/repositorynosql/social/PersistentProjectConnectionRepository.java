...
public interface PersistentProjectConnectionRepository extends MongoRepository<PersistentProjectConnection, Long> {
    ...
    Page<PersistentProjectConnection> findAllByUserAndProjectAndCreatedLessThan(Long user,
                                                                                Long project,
                                                                                Date created,
                                                                                PageRequest pageRequest);


    @Aggregation(pipeline = {"{$match: {project: ?0, user: ?1, $and : [{created: {$gte: ?3}}," +
        "{created: {$lte: ?2}}]}},{$sort: {created: 1}},{$project: {dateInMillis: {$subtract: " +
        "{'$created', ?4}}}}"})
    AggregationResults test(Long project, Long user, Date before, Date after, Date firstDate);

    Page<PersistentProjectConnection> findAllByUserAndProject(Long user, Long project,
                                                              PageRequest pageRequest);
