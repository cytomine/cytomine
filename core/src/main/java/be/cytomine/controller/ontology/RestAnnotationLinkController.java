...
public class RestAnnotationLinkController extends RestCytomineController {
    ...
    public ResponseEntity<String> listByAnnotationGroup(@PathVariable Long id) {
        ...
        AnnotationGroup annotationGroup =
            annotationGroupService.find(id).orElseThrow(() -> new ObjectNotFoundException(
                "AnnotationGroup", id));
    ...
    public ResponseEntity<String> show(@PathVariable Long annotation, @PathVariable Long
    annotationGroup) {
        log.debug("REST request to get annotation {} in annotationgroup {}", annotation,
        annotationGroup);
        ...
        if (link == null) {
            return responseNotFound("AnnotationLink", Map.of("Annotation", annotation,
            "AnnotationGroup", annotationGroup));
        }
    ...
    public ResponseEntity<String> delete(@PathVariable Long annotation,
                                         @PathVariable Long annotationGroup) {
        log.debug("REST request to delete annotation {} in annotationgroup {}", annotation,
            annotationGroup);

        AnnotationLink link =
            annotationLinkRepository.findByAnnotationIdent(annotation).orElse(null);
        if (link == null) {
            return responseNotFound("Annotationlink", Map.of("Annotation", annotation,
                "AnnotationGroup", annotationGroup));
        }

        return responseSuccess(annotationLinkService.delete(link, transactionService.start(),
            null, true));
    }
}
