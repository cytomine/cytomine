...
public class GenericAnnotationService {
     ...
     */
    public List<AnnotationDomain> findAnnotationThatTouch(String location, List<Long> layers,
                                                          long idImage, String table) {
        ...
        if (!projectAdmin) {
            layers = layers.stream().filter(x -> Objects.equals(x,
        ...
        if (table.equals("reviewed_annotation")) {
            results =
                annotationDomainRepository.findAllIntersectForReviewedAnnotations(image.getId(),
                    location);
        } else {
            results = annotationDomainRepository.findAllIntersectForUserAnnotations(image.getId()
                , layers, location);
        }
        ...
        if (users.size() > 1 && !table.equals("reviewed_annotation")) { //if more annotation from
            // more than 1 user NOT IN REVIEW MODE!
            throw new WrongArgumentException("Annotations from multiple users are under this area" +
...
//            annotations = new ArrayList<>();
     ...
     */
    public List<ReviewedAnnotation> findReviewedAnnotationWithTerm(List<Long> ids,
                                                                   List<Long> termsId) {
