...
export default {
  ...
  props: {
    ...
    hitTolerance: {type: Number, default: 0}
  },
  emits: ['translatestart', 'translateend'],
  created() {
    this.$interaction = new Translate({
      features: getIdent(this.source),
      hitTolerance: this.hitTolerance
    });
    this.$interaction.on('translatestart', evt => this.$emit('translatestart', evt));
    this.$interaction.on('translateend', evt => this.$emit('translateend', evt));
