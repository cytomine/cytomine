...
public class AnnotationActionServiceTests {
    ...
    public void add_action_for_annotation() {
        ...
        given_a_persistent_annotation_action(new Date(), builder.given_a_user_annotation(),
            builder.given_superadmin(), "view");
    ...
    public void list_by_slice() {
        ...
        given_a_persistent_annotation_action(new Date(), annotationDomain,
            builder.given_superadmin(), "view");
        given_a_persistent_annotation_action(new Date(), annotationDomain,
        ...
        assertThat(annotationActionService.list(annotationDomain.getSlice(),
            builder.given_superadmin(), null, null))
            .hasSize(2);

        assertThat(annotationActionService.list(annotationDomain.getSlice(),
            builder.given_a_user(), null, null))
            .hasSize(0);

        assertThat(annotationActionService.list(annotationDomain.getSlice(),
            builder.given_superadmin(), null, new Date().getTime()))
            .hasSize(2);

        assertThat(annotationActionService.list(annotationDomain.getSlice(),
            builder.given_superadmin(), new Date().getTime(), null))
            .hasSize(0);
    }
    ...
    void total_annotation_action_by_project_count() {
        ...
        given_a_persistent_annotation_action(DateUtils.addDays(new Date(), -10), annotationDomain
            , user1, "select");
        given_a_persistent_annotation_action(DateUtils.addDays(new Date(), -10), annotationDomain
            , user1, "view");
        Date twoConnectionBefore = DateUtils.addDays(new Date(), -5);
        given_a_persistent_annotation_action(DateUtils.addDays(new Date(), -3), annotationDomain,
            user1, "select");
