...
public class UserPositionServiceTests {
    ...
    void user_position_create_persistent_and_expired_position() {
        PersistentUserPosition persistentUserPosition =
            given_a_persistent_user_position(new Date(), builder.given_superadmin(),
    ...
    void retrieve_last_position_for_user() {
        ...
        PersistentUserPosition persistentUserPosition =
            given_a_persistent_user_position(new Date(), mainUser, sliceInstance);
        ...
        PersistentUserPosition persistentUserPositionForAnotherUserAnotherSlice
            = given_a_persistent_user_position(new Date(), anotherUser,
            builder.given_a_slice_instance());

        Optional<LastUserPosition> lastUserPosition
            = userPositionService.lastPositionByUser(sliceInstance.getImage(), sliceInstance,
            mainUser, false);
    ...
    public void list_users_online_on_image() {
        ...
        assertThat(userPositionService.listOnlineUsersByImage(sliceInstance.getImage(),
            sliceInstance, false))
    ...
    public void summerize_location() {
        ...
        given_a_persistent_user_position(DateUtils.addSeconds(oldPosition, 1), mainUser,
            sliceInstance, USER_VIEW);
        given_a_persistent_user_position(DateUtils.addSeconds(oldPosition, 2), mainUser,
            sliceInstance, USER_VIEW);
        given_a_persistent_user_position(DateUtils.addSeconds(oldPosition, 3), mainUser,
            sliceInstance, USER_VIEW);
        given_a_persistent_user_position(DateUtils.addSeconds(oldPosition, 4), mainUser,
            sliceInstance, ANOTHER_USER_VIEW);
        ...
        Optional<Map<String, Object>> first =
            summarize.stream().filter(x -> ((List<List<Double>>) x.get("location")).get(0).contains(USER_VIEW.toList().get(0).get(0))).findFirst();
        assertThat(first).isPresent();
        assertThat(first.get().get("frequency")).isEqualTo(3);
        Optional<Map<String, Object>> second =
            summarize.stream().filter(x -> ((List<List<Double>>) x.get("location")).get(0).contains(ANOTHER_USER_VIEW.toList().get(0).get(0))).findFirst();
    ...
    public void summerize_after_than() {
        ...
        given_a_persistent_user_position(DateUtils.addDays(oldPosition, 1), mainUser,
            sliceInstance, USER_VIEW);
        given_a_persistent_user_position(DateUtils.addDays(oldPosition, 3), mainUser,
            sliceInstance, USER_VIEW);
        given_a_persistent_user_position(DateUtils.addDays(oldPosition, 5), mainUser,
            sliceInstance, USER_VIEW);
        given_a_persistent_user_position(DateUtils.addDays(oldPosition, 7), mainUser,
            sliceInstance, USER_VIEW);
    ...
    void adding_a_position_with_new_location() {
        ...
        AreaDTO area = new AreaDTO(new Point((double) 0, (double) 0), new Point((double) 0,
            (double) 0), new Point((double) 0, (double) 0), new Point((double) 0, (double) 0));
        Date date = new Date();

        userPositionService.add(date, user, sliceInstance, imageInstance, area, 0, (double) 0,
            true);
    }
    ...
    public void list_followers() {
        ...
        WebSocketUserPositionHandler.sessionsBroadcast.put(user.getId().toString() + "/514",
            sessionDecorator);
        WebSocketUserPositionHandler.sessionsTracked.put(sessionDecorator,
            new ConcurrentWebSocketSessionDecorator[]{new ConcurrentWebSocketSessionDecorator(session, 0, 0)});
        WebSocketUserPositionHandler.sessions.put(user.getId().toString(),
            new ConcurrentWebSocketSessionDecorator[]{new ConcurrentWebSocketSessionDecorator(session, 0, 0)});
