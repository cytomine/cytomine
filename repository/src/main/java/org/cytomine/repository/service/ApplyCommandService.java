        ...
        return commandRepository.findById(undoCommand).flatMap(commandEntity -> switch (commandEntity.getData()) {
            ...
            case UndoDeleteCommand<?> v -> switch (v.target()) {
                ...
                case DeleteTermCommand dtc ->
                    termCommandService.undoCreate(v.commandId(), new CreateTermCommand(dtc.before(), userId), userId,
                        now);
                case DeleteOntologyCommand doc ->
                    ontologyCommandService.undoCreate(v.commandId(), new CreateOntologyCommand(doc.before(), userId),
                        userId, now);
                case DeleteTagDomainAssociationCommand dtdac ->
