...
public class PersistentUserPosition extends CytomineSocialDomain {
    ...
    // TODO:
    // stateless true //don't store data in memory after read&co. These data don't need to be
    // update.
    ...
    public static Polygon getJtsPolygon(GeoJsonPolygon polygon) {
        List<Coordinate> coordinates =
            polygon.getPoints().stream().map(point -> new Coordinate(point.getX(), point.getY())).collect(Collectors.toList());
    ...
    public static Polygon getJtsPolygon(List<List<Double>> polygon) {
        List<Coordinate> coordinates = polygon.stream().map(point -> new Coordinate(point.get(0),
            point.get(1))).collect(Collectors.toList());
        if (coordinates.size() > 1) {
            // finish with the first point
            coordinates.add(coordinates.get(0));
        }
