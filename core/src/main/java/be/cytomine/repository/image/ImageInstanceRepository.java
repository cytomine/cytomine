 ...
 */
@Repository
public interface ImageInstanceRepository extends JpaRepository<ImageInstance, Long>,
    JpaSpecificationExecutor<ImageInstance> {
    ...
    default List<ImageInstance> findAllByBaseImage(AbstractImage abstractImage,
                                                   Predicate<ImageInstance> predicate) {
        return findAllByBaseImage(abstractImage).stream().filter(predicate).collect(Collectors.toList());
    }

    @Query(value = "SELECT a.id FROM image_instance a WHERE project_id=:projectId AND parent_id " +
        "IS NULL", nativeQuery = true)
    List<Long> getAllImageId(Long projectId);

    @Query(value = "SELECT a FROM image_instance a WHERE project_id=:projectId AND " +
        "base_image_id=:baseImageId AND parent_id IS NULL", nativeQuery = true)
    ...
    Optional<ImageInstance> findTopByProjectAndCreatedLessThanOrderByCreatedDesc(Project project,
                                                                                 Date created);
