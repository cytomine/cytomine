...
public class ProjectRepresentativeUserResourceTests {
    ...
    public void list_all_project_representative_users() throws Exception {
        ...
        restProjectRepresentativeUserControllerMockMvc.perform(get("/api/project/{id" +
                "}/representative.json", projectRepresentativeUser.getProject().getId()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection", hasSize(greaterThan(0))))
            .andExpect(jsonPath("$.collection[?(@.id=='" + projectRepresentativeUser.getId() +
                "')]").exists());
    }
    ...
    public void get_a_projectRepresentativeUser() throws Exception {
        ...
        restProjectRepresentativeUserControllerMockMvc.perform(get("/api/project/{project" +
                    "}/representative/{id}.json",
            ...
            .andExpect(jsonPath("$.class").value("be.cytomine.domain.project" +
    ...
    public void add_valid_projectRepresentativeUser() throws Exception {
        ...
        restProjectRepresentativeUserControllerMockMvc.perform(post("/api/project/{id" +
                "}/representative.json", projectRepresentativeUser.getProject().getId())
            ...
            .andExpect(jsonPath("$.callback.method").value("be.cytomine" +
                ".AddProjectRepresentativeUserCommand"))
    ...
    public void delete_projectRepresentativeUser() throws Exception {
        ...
        restProjectRepresentativeUserControllerMockMvc.perform(delete("/api/project/{project" +
                    "}/representative/{id}.json",
                projectRepresentativeUser.getProject().getId(), projectRepresentativeUser.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(projectRepresentativeUser.toJSON()))
            ...
            .andExpect(jsonPath("$.callback").exists())
            .andExpect(jsonPath("$.callback.method").value("be.cytomine" +
                ".DeleteProjectRepresentativeUserCommand"))
            .andExpect(jsonPath("$.message").exists())
            .andExpect(jsonPath("$.command").exists());
    }
    ...
    public void fail_when_delete_projectRepresentativeUser_user_not_exists() throws Exception {
        restProjectRepresentativeUserControllerMockMvc.perform(delete("/api/project/{project" +
                "}/representative.json", builder.given_a_project().getId())
                .param("user", "0")
                .contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isNotFound())
            .andExpect(jsonPath("$.errors").exists());
