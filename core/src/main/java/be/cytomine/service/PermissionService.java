...
public class PermissionService {
    ...
    public boolean hasExactACLPermission(CytomineDomain domain, String username,
    ...
    public boolean hasACLPermission(CytomineDomain domain, Permission permission, boolean isAdmin) {
        return isAdmin || hasACLPermission(domain, currentUserService.getCurrentUsername(),
            permission);
    }
    ...
    public void deletePermission(CytomineDomain domain, String username, Permission permission) {
        log.debug("Current mask for user {} on domain {} before request: {}", username,
            domain.getId(), aclRepository.listMaskForUsers(domain.getId(), username));
        if (hasACLPermission(domain, username, permission)) {
            log.info("Delete permission for {}, {}, {}", username, permission.getMask(),
                domain.getId());
        ...
        log.debug("Current mask for user {} on domain {} after request: {}", username,
            domain.getId(), aclRepository.listMaskForUsers(domain.getId(), username));
    }
    ...
    public void addPermissionOptimised(Long aclObjectIdentity, String username,
                                       Permission permission, Integer index) {
        //get acl sid for the user
        Long sid = getAclSid(username);
    ...
    public Long createAclEntry(Long aoi, Long sid, Integer mask) {
        ...
        synchronized (this.getClass()) {
            ...
            // the synchronize does not resolve fully the issue since the INSERT may not be
            // persisted in database directly (batch_insert / flush / ...)
