...
describe('CytomineSlider.vue', () => {
  ...
  describe('rendering', () => {
    it('should render a single thumb for a scalar value', () => {
      const wrapper = createWrapper({ value: 30 });
    ...
    it('should not render tooltips when tooltip is disabled', () => {
      const wrapper = createWrapper({ value: 30, tooltip: false });
  ...
  describe('missing / invalid values (no NaN)', () => {
    ...
    it('should fill the upper bound with max when it is not yet loaded', () => {
      const wrapper = createWrapper({ value: [MIN, undefined] });
    ...
    it('should default an empty range to [min, max]', () => {
      const min = 5;
      const max = 42;
      const wrapper = createWrapper({ value: [undefined, undefined], min, max });
  ...
  describe('computed geometry', () => {
    it('should compute the process bar for a range', () => {
      const wrapper = createWrapper({ value: [20, 60] });
    ...
    it('should compute the process bar for a scalar value from the start', () => {
      const wrapper = createWrapper({ value: 40 });
    ...
    it('should guard against a zero range', () => {
      const bound = 50;
      const wrapper = createWrapper({ value: bound, min: bound, max: bound });

      expect(wrapper.vm.percent(bound)).toBe(0);
    });

    it('should place single-value tooltip on the side away from the thumb', () => {
      expect(createWrapper({ value: 90 }).vm.tooltipPlacement).toEqual(['left']);
      expect(createWrapper({ value: 10 }).vm.tooltipPlacement).toEqual(['right']);
    });
  ...
  describe('keyboard interaction', () => {
    it('should increment a scalar value on ArrowRight and emit input', async () => {
      const wrapper = createWrapper({ value: 30 });
    ...
    it('should decrement on ArrowDown', async () => {
      ...
      expect(wrapper.emitted('input')[0]).toEqual([29]);
    });
    ...
    it('should respect the interval as the keyboard step', async () => {
      const wrapper = createWrapper({ value: 30, interval: 5 });

      await dots(wrapper).at(0).trigger('keydown', { key: 'ArrowRight' });

      expect(wrapper.emitted('input')[0]).toEqual([35]);
    });

    it('should clamp at the maximum bound', async () => {
      const wrapper = createWrapper({ value: MAX });

      await dots(wrapper).at(0).trigger('keydown', { key: 'ArrowRight' });

      expect(wrapper.emitted('input')[0]).toEqual([MAX]);
    });
  ...
  describe('range: thumbs cannot cross', () => {
    it('should clamp the lower thumb to the upper value', () => {
      const wrapper = createWrapper({ value: [40, 60] });
  ...
  describe('inline edition', () => {
    ...
    it('should reorder bounds when the edited value crosses the other thumb', () => {
      ...
      expect(wrapper.emitted('input')[0][0]).toEqual([80, 90]);
    });

    it('should parse to an integer when integerOnly is set', () => {
      const wrapper = createWrapper({ value: 30, integerOnly: true });
    ...
    it('should keep decimals when integerOnly is false', () => {
      const wrapper = createWrapper({ value: 30, integerOnly: false });

      wrapper.vm.startEdition(0);
      wrapper.vm.editedValue = '42.7';
      wrapper.vm.stopEdition(0);

      expect(wrapper.emitted('input')[0]).toEqual([42.7]);
    });
  ...
  describe('snapping', () => {
    ...
    it('should snap to the nearest interval multiple', () => {
      const wrapper = createWrapper({ value: 0, interval: 5 });
    ...
    it('should clamp snapped values within [min, max]', () => {
      const min = 10;
      const max = 20;
      const wrapper = createWrapper({ value: min, min, max });
  ...
  describe('v-model contract (compat: value / input)', () => {
    it('should reset internalValue when the value prop changes', async () => {
      const wrapper = createWrapper({ value: [10, 80] });

      await wrapper.setProps({ value: [20, 90] });

      expect(wrapper.vm.internalValue).toEqual([20, 90]);
    });

    it('should emit the "input" event (not update:modelValue)', async () => {
      const wrapper = createWrapper({ value: 30 });
