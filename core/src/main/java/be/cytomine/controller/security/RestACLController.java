            ...
            if (domainClassName != null && domainIdent != null && user != null) {
                ...
                throw new ObjectNotFoundException(
                    "Request not valid: domainClassName="
                        ...
                        + domainIdent + ", user=" + user
                );
            }
        } catch (CytomineException e) {
            return ResponseEntity.status(e.code).contentType(MediaType.APPLICATION_JSON)
                .body(JsonObject.of("success", false, "errors", e.msg).toJsonString());
        }
    }
}
