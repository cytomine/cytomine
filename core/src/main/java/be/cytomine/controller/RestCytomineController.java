...
public abstract class RestCytomineController {
    ...
    protected Pageable retrievePageable() {
        ...
        int realMax = requestParams.get("max").equals("0")
            ? Integer.MAX_VALUE
        ...
        for (Map.Entry<String, String[]> entry : parameterMap.entrySet()) {
            if (entry.getValue() != null && entry.getValue().length > 1) {
                throw new CytomineMethodNotYetImplementedException(
                    "Multiple request params are not supported in this method"
                );
    ...
    private JsonObject buildJsonList(List list, Integer offsetParameter, Integer maxParameter) {
        ...
        return JsonObject.of(
            "collection", subList,
            "offset", offset,
            "perPage", Math.min(max, list.size()),
            "size", list.size(),
            "totalPages", (int) Math.ceil((double) list.size() / (double) max)
        );
    ...
    private JsonObject buildJsonList(Page page, Integer offsetParameter, Integer maxParameter) {
        ...
        return JsonObject.of(
            ...
            "offset", offset,
            "perPage", Math.min(max, page.getContent().size()),
            "size", page.getTotalElements(),
            "totalPages", (int) Math.ceil((double) page.getTotalElements() / (double) max)
        );
    }
    ...
    protected ResponseEntity<String> responseSuccess(Page page, boolean isFilterRequired) {
        ...
        if (isFilterRequired) {
            if (!page.getContent().isEmpty() && page.getContent().get(0) instanceof Map) {
                ...
                throw new CytomineMethodNotYetImplementedException(
                    "Filter is not working with this class type " + page.getContent().get(0).getClass()
                );
            }
        }
    ...
    protected ResponseEntity<String> responseSuccess(Page page, Integer offsetParameter, Integer maxParameter) {
        return JsonResponseEntity.status(HttpStatus.OK)
            .body(buildJsonList(page, offsetParameter, maxParameter).toJsonString());
    }
    ...
    public ResponseEntity<String> responseSuccess(
        List list, Integer offsetParameter, Integer maxParameter, boolean isFilterRequired
    ) {
        List filtered = list;
        if (isFilterRequired) {
        ...
        if (filtered.isEmpty() || filtered.get(0) instanceof CytomineDomain) {
            ...
            return responseSuccessSocialDomainList(
                (List<? extends CytomineSocialDomain>) filtered, offsetParameter, maxParameter
            );
        } else {
    ...
    public ResponseEntity<String> responseSuccess(List list, boolean isFilterRequired) {
        ...
        return responseSuccess(
            list, requestParams.getOffset().intValue(), requestParams.getMax().intValue(), isFilterRequired
        );
    }
    ...
    private ResponseEntity<String> responseSuccessDomainList(
        List<? extends CytomineDomain> list, Integer offsetParameter, Integer maxParameter
    ) {
        //TODO: perf convert after buildJsonList will avoid converting unused items (out of page)
        return JsonResponseEntity.status(HttpStatus.OK)
            .body(buildJsonList(convertCytomineDomainListToJSON(list), offsetParameter, maxParameter).toJsonString());
    }

    private ResponseEntity<String> responseSuccessSocialDomainList(
        List<? extends CytomineSocialDomain> list, Integer offsetParameter, Integer maxParameter
    ) {
        return JsonResponseEntity.status(HttpStatus.OK)
            .body(
                buildJsonList(convertCytomineSocialDomainListToJSON(list), offsetParameter, maxParameter).toJsonString()
            ); //TODO: perf convert after buildJsonList will avoid converting unused items (out of page)
    }

    private ResponseEntity<String> responseSuccessGenericList(
        List list, Integer offsetParameter, Integer maxParameter
    ) {
        //TODO: perf convert after buildJsonList will avoid converting unused items (out of page)
        return JsonResponseEntity.status(HttpStatus.OK)
