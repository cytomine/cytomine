...
public interface CommandRepository extends JpaRepository<Command, Long> {
    ...
    @Query("SELECT usi FROM UndoStackItem usi WHERE usi.transaction = :transaction AND usi.user =" +
        " :user ORDER BY usi.created DESC")
    List<UndoStackItem> findAllUndoOrderByCreatedDesc(User user, Transaction transaction);

    @Query("SELECT rsi FROM RedoStackItem rsi WHERE rsi.transaction = :transaction AND rsi.user =" +
        " :user ORDER BY rsi.created DESC")
    List<RedoStackItem> findAllRedoOrderByCreatedDesc(User user, Transaction transaction);


    @Query("SELECT usi FROM UndoStackItem usi WHERE usi.command = :command AND usi.user = :user " +
    ...
    @Query("SELECT usi FROM RedoStackItem usi WHERE usi.command = :command AND usi.user = :user " +
        "ORDER BY usi.created DESC")
