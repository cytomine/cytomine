...
public class AbstractSliceService extends ModelService {
    ...
    public Optional<AbstractSlice> find(AbstractImage abstractImage, Integer channel, Integer zStack, Integer time) {
        Optional<AbstractSlice> abstractSlice = abstractSliceRepository.findByImageAndChannelAndZStackAndTime(
            abstractImage,
            channel,
            zStack,
            time
    ...
    public boolean isAbstractSliceUsed(Long abstractImageId) {
        AbstractSlice domain = find(abstractImageId).orElseThrow(() -> new ObjectNotFoundException(
            "AbstractImage",
            abstractImageId
        ));
    ...
    public List<Object> getStringParamsI18n(CytomineDomain domain) {
        return List.of(
            domain.getId(),
            ((AbstractSlice) domain).getChannel(),
            ((AbstractSlice) domain).getZStack(),
            ((AbstractSlice) domain).getTime()
        );
    }
    ...
    private void deleteDependentSliceInstance(AbstractSlice ai, Transaction transaction, Task task) {
        ...
        if (!images.isEmpty()) {
            throw new ConstraintException("This slice "
                + ai.getId()
                + " cannot be deleted as it has already been insert "
                +
                "in projects "
    ...
    public void checkDoNotAlreadyExist(CytomineDomain domain) {
        ...
        abstractSliceRepository.findByImageAndChannelAndZStackAndTime(
            abstractSlice.getImage(),
            abstractSlice.getChannel(),
            abstractSlice.getZStack(),
            abstractSlice.getTime()
        ).ifPresent(slice -> {
            if (!Objects.equals(slice.getId(), abstractSlice.getId())) {
                throw new AlreadyExistException("AbstractSlice (C:"
                    + abstractSlice.getChannel()
                    + ", Z:"
                    + abstractSlice.getZStack()
                    + ", T:"
                    + abstractSlice.getTime()
                    + ") already exists for AbstractImage "
                    + abstractSlice.getImage().getId());
            }
        });
