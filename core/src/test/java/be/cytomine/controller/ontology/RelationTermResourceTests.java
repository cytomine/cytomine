...
public class RelationTermResourceTests {
    ...
    public void list_by_term_position_1() throws Exception {
        ...
        restRelationTermControllerMockMvc.perform(get("/api/relation/term/{i}/{id}.json", "1",
    ...
    public void list_by_term_position_2() throws Exception {
        RelationTerm relationTerm = builder.given_a_relation_term();
        restRelationTermControllerMockMvc.perform(get("/api/relation/term/{i}/{id}.json", "2",
                relationTerm.getTerm1().getId()))
            .andExpect(status().isOk())
    ...
    public void list_by_term() throws Exception {
        RelationTerm relationTerm = builder.given_a_relation_term();
        restRelationTermControllerMockMvc.perform(get("/api/relation/term/{id}.json",
                relationTerm.getTerm1().getId()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection", hasSize(1)))
            .andExpect(jsonPath("$.collection[?(@.id=='" + relationTerm.getId() + "')]").exists());
    ...
    public void get_a_relation_term() throws Exception {
        ...
        restRelationTermControllerMockMvc.perform(get("/api/relation/{idRelation}/term1/{idTerm1" +
                    "}/term2/{idTerm2}.json", relationTerm.getRelation().getId(),
                relationTerm.getTerm1().getId(), relationTerm.getTerm2().getId()))
            .andExpect(status().isOk())
    ...
    public void get_a_parent_relation_term() throws Exception {
        ...
        restRelationTermControllerMockMvc.perform(get("/api/relation/parent/term1/{idTerm1}/term2" +
                    "/{idTerm2}.json", relationTerm.getTerm1().getId(),
    ...
    public void delete_relation_term() throws Exception {
        RelationTerm relationTerm = builder.given_a_relation_term();
        restRelationTermControllerMockMvc.perform(delete("/api/relation/{idRelation}/term1" +
                    "/{idTerm1}/term2/{idTerm2}.json", relationTerm.getRelation().getId(),
                relationTerm.getTerm1().getId(), relationTerm.getTerm2().getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(relationTerm.toJSON()))
            .andExpect(status().isOk())
    ...
    public void delete_parent_relation_term() throws Exception {
        RelationTerm relationTerm = builder.given_a_relation_term();
        restRelationTermControllerMockMvc.perform(delete("/api/relation/parent/term1/{idTerm1" +
                    "}/term2/{idTerm2}.json", relationTerm.getTerm1().getId(),
                relationTerm.getTerm2().getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(relationTerm.toJSON()))
            .andExpect(status().isOk())
