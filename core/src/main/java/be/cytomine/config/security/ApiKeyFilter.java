...
public class ApiKeyFilter extends OncePerRequestFilter {
    ...
    public static String generateKeys(String method, String content_md5, String content_type,
                                      String date, String queryString, String path, User user) throws NoSuchAlgorithmException, InvalidKeyException, UnsupportedEncodingException {
    ...
    private boolean tryAPIAuthentification(HttpServletRequest request) {
        ...
        try {
            ...
            String content_md5 = (request.getHeader("content-MD5") != null) ? request.getHeader(
                "content-MD5") : "";

            String content_type = (request.getHeader("content-type") != null) ?
                request.getHeader("content-type") : "";
            content_type = (request.getHeader("Content-Type") != null) ? request.getHeader(
            ...
            String accessKey = authorization.substring(authorization.indexOf(" ") + 1,
            ...
            if (user.isEmpty()) {
                ...
                String signature = generateKeys(request.getMethod(), content_md5, content_type,
                    date, queryString, path, user.get());
                if (authorizationSign.equals(signature)) {
                    this.reauthenticate(user.get());
                    return true;
                } else {
                    // the java client does not set content-type, so we override the header to
                    // application/json BEFORE this authentication.
                    // So the client thinks content-type is "" while spring boot set it to
                    // application/json. In order to match the client signature, we generate it
                    // with an empty value.
                    // => it would be better to improve the java client to set a valid content type.
                    String signatureWithEmptyContentType = generateKeys(request.getMethod(),
                        content_md5, "", date, queryString, path, user.get());
    ...
    private org.springframework.security.core.userdetails.User createSpringSecurityUser(User user) {
        if (!user.getEnabled()) {
            throw new ForbiddenException("User with access key " + user.getPublicKey() + "is not " +
                "enabled.");
        }
