...
public class AnnotationListingService {
    ...
    public List listGeneric(AnnotationListing al) {
        ...
        if ((al.getKmeans() != null && al.getKmeans()) && al.getKmeansValue() == null) {
            if (al.getBbox() == null) {
                throw new WrongArgumentException("If you want to use kmeans, you must provide " +
                    "image bbox:" + al.getBbox());
            }
            ...
            al.setSlices((al.getSlices() == null || al.getSlices().isEmpty()) ?
                List.of(al.getSlice()) : al.getSlices());
            Integer rule = kmeansGeometryService.mustBeReduce(al.getSlices(), al.getUser(),
                al.getBbox());
    ...
    private List<AnnotationResult> selectGenericAnnotation(AnnotationListing al) {
        ...
        for (Tuple rowResult : resultList) {
             ...
             * For the first line for this annotation (it.id!=lastAnnotationId), add the
             * annotation data,
            ...
            if ((Long) tuple.get("id") != lastAnnotationId) {
                ...
                for (String columnName : realColumn) {
                    item.put(columnName, tuple.get(columnName) != null ? tuple.get(columnName) :
                        tuple.get(columnName.toLowerCase()));
                }


                if (al.getColumnsToPrint().contains("term")) {
                    termAsked = true;
                    item.put("term", tuple.get("term") != null ? buildList(tuple.get("term")) :
                        new ArrayList<>());
                    item.put("userByTerm",
                        tuple.get("term") != null ?
                            buildList(new HashMap<>(JsonObject.of("id", tuple.get(
                                    "annotationterms"), "term", tuple.get("term"), "user",
                                buildList(tuple.get("userterm"))))) : new ArrayList<>());
                }

                if (al.getColumnsToPrint().contains("track") && (al instanceof UserAnnotationListing)) {
                    trackAsked = true;
                    item.put("track", (tuple.get("track") != null ?
                        buildList(tuple.get("track")) : new ArrayList<>()));
                    item.put("annotationTrack", (tuple.get("track") != null ?
                        buildList(new HashMap<>(Map.of("id", tuple.get("annotationtracks"),
                            "track", tuple.get("track")))) : new ArrayList<>()));
                }

                if (al.getColumnsToPrint().contains("group")) {
                    groupAsked = true;
                    item.put("group", tuple.get("group"));
                    item.put("annotationLink", (tuple.get("group") != null ?
                        buildList(new HashMap<>(Map.of(
                ...
                if (al.getColumnsToPrint().contains("gis")) {
                    item.put("perimeterUnit", tuple.get("perimeterunit") != null ?
                        GisUtils.retrieveUnit((Integer) tuple.get("perimeterunit")) : null);
                    item.put("areaUnit", tuple.get("areaunit") != null ?
                        GisUtils.retrieveUnit((Integer) tuple.get("areaunit")) : null);
                ...
                if (al.getColumnsToPrint().contains("meta")) {
                    if (al.getClass().getName().contains("UserAnnotation")) {
                        item.put("cropURL",
                            UrlApi.getUserAnnotationCropWithAnnotationId((Long) tuple.get("id"),
                                "png"));
                        item.put("smallCropURL",
                            UrlApi.getUserAnnotationCropWithAnnotationIdWithMaxSize((Long) tuple.get("id"), 256, "png"));
                        item.put("url",
                            UrlApi.getUserAnnotationCropWithAnnotationId((Long) tuple.get("id"),
                                "png"));
                        item.put("imageURL", UrlApi.getAnnotationURL((Long) tuple.get("project"),
                            (Long) tuple.get("image"), (Long) tuple.get("id")));
                    } else if (al.getClass().getName().contains("ReviewedAnnotation")) {
                        item.put("cropURL",
                            UrlApi.getReviewedAnnotationCropWithAnnotationId((Long) tuple.get("id"
                            ), "png"));
                        item.put("smallCropURL",
                            UrlApi.getReviewedAnnotationCropWithAnnotationIdWithMaxSize((Long) tuple.get("id"), 256, "png"));
                        item.put("url",
                            UrlApi.getReviewedAnnotationCropWithAnnotationId((Long) tuple.get("id"
                            ), "png"));
                        item.put("imageURL", UrlApi.getAnnotationURL((Long) tuple.get("project"),
                ...
                if (termAsked && tuple.get("term") != null) {
                    ...
                    Map userByTerm =
                        (Map) (lasResultUserByTerm).get(lasResultUserByTerm.size() - 1);
                    ...
                    if ((Long) tuple.get("term") == lastTermId) {
                        ...
                        ((List) lastResult.get("userByTerm")).add(new HashMap<>(Map.of("id",
                            tuple.get("annotationterms"), "term", tuple.get("term"), "user",
                ...
                if (trackAsked && tuple.get("track") != null && (long) tuple.get("track") != lastTrackId && !((List) lastResult.get("track")).contains(tuple.get("track"))) {
                    ...
                    ((List) lastResult.get("annotationTrack")).add(new HashMap<>(Map.of("id",
                        tuple.get("annotationtracks"), "track", tuple.get("track"))));
                }
