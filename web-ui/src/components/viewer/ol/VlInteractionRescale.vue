...
export default {
  ...
  emits: ['rescalestart', 'rescaleend'],
  created() {
    this.$interaction = new RescaleFeatureInteraction({
      features: getIdent(this.source),
      factor: this.factor
    });
    this.$interaction.on('rescalestart', evt => this.$emit('rescalestart', evt));
    this.$interaction.on('rescaleend', evt => this.$emit('rescaleend', evt));
