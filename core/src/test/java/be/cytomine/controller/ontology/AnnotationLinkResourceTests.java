...
public class AnnotationLinkResourceTests {
    ...
    public void list_annotation_link_by_annotation_group() throws Exception {
        ...
        restAnnotationLinkControllerMockMvc.perform(get("/api/annotationgroup/{id}/annotationlink" +
                ".json", annotationLink.getGroup().getId()))
            .andExpect(status().isOk())
    ...
    public void list_annotation_link_by_annotation() throws Exception {
        AnnotationLink annotationLink = builder.given_an_annotation_link();
        restAnnotationLinkControllerMockMvc.perform(get("/api/annotation/{id}/annotationlink" +
                ".json", annotationLink.getAnnotationIdent()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection[?(@.id==" + annotationLink.getId() + ")]").exists());
    }
    ...
    public void delete_annotation_link() throws Exception {
        AnnotationLink annotationLink = builder.given_an_annotation_link();
        restAnnotationLinkControllerMockMvc.perform(delete("/api/annotationgroup/{annotationGroup" +
                    "}/annotation/{annotation}.json", annotationLink.getGroup().getId(),
