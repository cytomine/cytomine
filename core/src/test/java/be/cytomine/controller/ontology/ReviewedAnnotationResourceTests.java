...
public class ReviewedAnnotationResourceTests {
    ...
    public void get_a_reviewed_annotation() throws Exception {
        ...
        restReviewedAnnotationControllerMockMvc.perform(get("/api/reviewedannotation/{id}.json",
    ...
    public void stats_annotations() throws Exception {
        ReviewedAnnotation reviewedAnnotation = builder.given_a_reviewed_annotation();
        restReviewedAnnotationControllerMockMvc.perform(get("/api/imageinstance/{image" +
                "}/reviewedannotation/stats.json", reviewedAnnotation.getImage().getId()))
            .andExpect(status().isOk());
    }
    ...
    public void count_annotations_by_project() throws Exception {
        ReviewedAnnotation reviewedAnnotation = builder.given_a_reviewed_annotation();
        restReviewedAnnotationControllerMockMvc.perform(get("/api/project/{idUser" +
                "}/reviewedannotation/count.json", reviewedAnnotation.getProject().getId()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.total").value(greaterThan(0)));

        Project projectWithoutAnnotation = builder.given_a_project();
        restReviewedAnnotationControllerMockMvc.perform(get("/api/project/{idUser" +
                "}/reviewedannotation/count.json", projectWithoutAnnotation.getId()))
            .andExpect(status().isOk())
    ...
    public void count_annotations_by_project_with_dates() throws Exception {
        ...
        restReviewedAnnotationControllerMockMvc.perform(get("/api/project/{idProject" +
                "}/reviewedannotation/count.json", oldReviewedAnnotation.getProject().getId())
    ...
    public void download_reviewed_annotation_pdf_document() throws Exception {
        ...
        restReviewedAnnotationControllerMockMvc.perform(get("/api/project/{project" +
                "}/reviewedannotation/download", this.project.getId())
                .param("format", "pdf")
                .param("reviewUsers", "")
    ...
    private void buildDownloadContext() throws ParseException {
        ...
        this.reviewedAnnotation = builder.given_a_reviewed_annotation(this.slice, "POLYGON((1 1,5" +
            " 1,5 5,1 5,1 1))", this.me, this.term);
        builder.addUserToProject(project, this.me.getUsername());
    }

    private MvcResult performDownload(String format) throws Exception {
        return restReviewedAnnotationControllerMockMvc.perform(get("/api/project/{project" +
                "}/reviewedannotation/download", this.project.getId())
                .param("format", format)
                .param("reviewUsers", "")
                .param("terms", this.term.getId().toString())
    ...
    private void checkResult(String delimiter, MvcResult result) throws UnsupportedEncodingException {
        TestUtils.checkSpreadsheetAnnotationResult(delimiter, result, this.reviewedAnnotation,
            this.project, this.image, this.me, this.term, "reviewedannotation",
            applicationProperties.getServerURL());
    }

    private void checkXLSResult(MvcResult result) throws IOException {
        TestUtils.checkSpreadsheetXLSAnnotationResult(result, this.reviewedAnnotation,
            this.project, this.image, this.me, this.term, "reviewedannotation",
            applicationProperties.getServerURL());
    }
    ...
    public void edit_valid_reviewed_annotation() throws Exception {
        ReviewedAnnotation reviewedAnnotation = builder.given_a_reviewed_annotation();
        restReviewedAnnotationControllerMockMvc.perform(put("/api/reviewedannotation/{id}.json",
                reviewedAnnotation.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(reviewedAnnotation.toJSON()))
            .andExpect(status().isOk())
    ...
    public void delete_reviewed_annotation() throws Exception {
        ReviewedAnnotation reviewedAnnotation = builder.given_a_reviewed_annotation();
        restReviewedAnnotationControllerMockMvc.perform(delete("/api/reviewedannotation/{id}" +
                ".json", reviewedAnnotation.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(reviewedAnnotation.toJSON()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.printMessage").value(true))
    ...
    public void start_image_review() throws Exception {
        ...
        restReviewedAnnotationControllerMockMvc.perform(post("/api/imageinstance/{image}/review" +
                ".json", imageInstance.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(""))
            .andExpect(status().isOk());
    ...
    public void stop_image_review() throws Exception {
        ...
        restReviewedAnnotationControllerMockMvc.perform(delete("/api/imageinstance/{image}/review" +
                ".json", imageInstance.getId()))
            .andExpect(status().isOk());
    ...
    public void add_annotation_review() throws Exception {
        ...
        restReviewedAnnotationControllerMockMvc.perform(post("/api/annotation/{annotation}/review" +
                ".json", userAnnotation.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(""))
            .andExpect(status().isOk());
    ...
    public void remove_annotation_review() throws Exception {
        ...
        restReviewedAnnotationControllerMockMvc.perform(delete("/api/annotation/{annotation" +
                "}/review.json", reviewedAnnotation.getParentIdent())
                .contentType(MediaType.APPLICATION_JSON)
                .content(""))
            .andExpect(status().isOk());
    ...
    public void review_full_layer() throws Exception {
        ...
        restReviewedAnnotationControllerMockMvc.perform(post("/api/imageinstance/{image" +
                "}/annotation/review.json", userAnnotation.getImage().getId())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    public void unreview_full_layer() throws Exception {
        ...
        restReviewedAnnotationControllerMockMvc.perform(delete("/api/imageinstance/{image" +
                "}/annotation/review.json", reviewedAnnotation.getImage().getId())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    public void get_reviewed_annotation_crop() throws Exception {
        ReviewedAnnotation annotation =
            given_a_reviewed_annotation_with_valid_image_server(builder);
        ...
        MvcResult mvcResult = restReviewedAnnotationControllerMockMvc.perform(get("/api" +
                "/reviewedannotation/{id}/crop.png?maxSize=512", annotation.getId()))
            .andExpect(status().isOk())
            .andReturn();
    ...
    public void get_reviewed_annotation_crop_mask() throws Exception {
        ...
        String url = "/image/" + URLEncoder.encode("1636379100999/CMU-2/CMU-2.mrxs",
            StandardCharsets.UTF_8).replace("%2F", "/") + "/annotation/mask";
        String body = "{\"level\":0,\"z_slices\":0,\"annotations\":[{\"geometry\":\"POLYGON ((1 " +
            "1, 50 10, 50 50, 10 50, 1 1))\",\"fill_color\":\"#fff\"}],\"timepoints\":0}";
        System.out.println(url);
        System.out.println(body);
        stubFor(WireMock.post(urlEqualTo(IMS_API_BASE_PATH + url)).withRequestBody(WireMock.equalTo(
                    body
        ...
        MvcResult mvcResult = restReviewedAnnotationControllerMockMvc.perform(get("/api" +
                "/reviewedannotation/{id}/mask.png", annotation.getId()))
            .andExpect(status().isOk())
            .andReturn();
        List<LoggedRequest> all = wireMockServer.findAll(RequestPatternBuilder.allRequests());
        AssertionsForClassTypes.assertThat(mvcResult.getResponse().getContentAsByteArray()).isEqualTo(mockResponse);
    ...
    public void get_reviewed_annotation_alpha_mask() throws Exception {
        ...
        configureFor("localhost", 8888);
        String url = "/image/" + URLEncoder.encode(annotation.getImage().getBaseImage().getPath()
            , StandardCharsets.UTF_8).replace("%2F", "/") + "/annotation/crop";
        String body = "{\"level\":0,\"z_slices\":0,\"annotations\":[{\"geometry\":\"POLYGON ((1 " +
            "1, 50 10, 50 50, 10 50, 1 1))\"}],\"timepoints\":0,\"background_transparency\":100}";
        System.out.println(url);
        System.out.println(body);
        stubFor(WireMock.post(urlEqualTo(IMS_API_BASE_PATH + url)).withRequestBody(WireMock.equalTo(
                    body
        ...
        MvcResult mvcResult = restReviewedAnnotationControllerMockMvc.perform(get("/api" +
                "/reviewedannotation/{id}/alphamask.png", annotation.getId()))
            .andExpect(status().isOk())
            .andReturn();
        List<LoggedRequest> all = wireMockServer.findAll(RequestPatternBuilder.allRequests());
        AssertionsForClassTypes.assertThat(mvcResult.getResponse().getContentAsByteArray()).isEqualTo(mockResponse);
