...
public class UploadedFileService extends ModelService {
    ...
    public Page<UploadedFile> list(User user, Long parentId, Boolean onlyRoot, Pageable pageable) {
        ...
        List<Storage> storages =
            securityACLService.getStorageList(currentUserService.getCurrentUser(), false);
        return uploadedFileRepository.search(user.getId(), parentId, onlyRoot,
            DomainUtils.extractIds(storages), pageable);
    }

    public List<Map<String, Object>> list(List<SearchParameterEntry> searchParameters,
                                          String sortedProperty, String sortDirection,
        ...
        for (SearchParameterEntry validatedSearchParameter : validatedSearchParameters) {
            ...
            if (validatedSearchParameter.getValue() instanceof CytomineDomain) {
                ...
                validatedSearchParameter.setProperty(validatedSearchParameter.getProperty() +
                    "_id");
            }
        ...
        validatedSearchParameters.addAll(
            SQLSearchParameter
                .getDomainAssociatedSearchParameters(AbstractImage.class, searchParameters,
                    getEntityManager())
                .stream()
                .filter(x -> !x.getProperty().equals("original_filename"))
                .map(x -> new SearchParameterEntry("ai." + x.getProperty(), x.getOperation(),
                    x.getValue()))
                .toList()
        );
        ...
        String search =
            sqlSearchConditions.getData().stream().map(SearchParameterEntry::getSql).collect(Collectors.joining(" AND "));

        String sort = "";
        if (List.of("content_type", "id", "created", "filename", "originalFilename", "size",
            "status").contains(sortedProperty)) {
        ...
        if (withTreeDetails) {
            ...
            treeJoin = "LEFT JOIN (SELECT *  FROM uploaded_file t " +
                "WHERE EXISTS (SELECT 1 FROM acl_sid AS asi LEFT JOIN acl_entry AS ae ON asi.id =" +
                " ae.sid " +
                "LEFT JOIN acl_object_identity AS aoi ON ae.acl_object_identity = aoi.id " +
                "WHERE aoi.object_id_identity = t.storage_id AND asi.sid = :username) AND t" +
                ".deleted IS NULL) " +
        ...
        String request = "SELECT uf.id, " +
            ...
            "ai.magnification, " +
            "ai.width, " +
            "CASE WHEN (nlevel(uf.l_tree) > 0) THEN ltree2text(subltree(uf.l_tree, 0, 1)) ELSE " +
            "NULL END AS root, " +
            treeSelect +
            "CASE WHEN (uf.status = " + UploadedFileStatus.CONVERTED.getCode() + " OR uf.status =" +
            ...
            "AND (uf.parent_id IS NULL OR parent.content_type IN ('" + String.join("','",
            UploadedFile.ARCHIVE_FORMATS) + "')) " +
        ...
        for (Tuple rowResult : resultList) {
            ...
            result.put("thumbURL", (result.get("image") != null) ?
                UrlApi.getAbstractImageThumbUrl((Long) result.get("image"), "png") : null);
    ...
    public List<Map<String, Object>> listHierarchicalTree(User user, Long rootId) {
        ...
        String request = "SELECT uf.id, uf.created, uf.original_filename, uf.content_type, " +
            ...
            "uf.size, uf.status, " +
            "cast (array_agg(ai.id) AS BIGINT[]) as image, cast (array_agg(asl.id) AS BIGINT[]) " +
            "as slices, cast (array_agg(cf.id) AS BIGINT[]) as companion_file " +
            "FROM uploaded_file uf " +
        ...
        for (Tuple rowResult : resultList) {
            ...
            result.put("slices", (Arrays.stream((Long[]) rowResult.get("slices"))).filter(x -> {
                ...
                boolean b = z != 0;
                return b;
            }).collect(Collectors.toList())); // A same UF can be linked to several slices
            // (virtual stacks)
            result.put("companionFile",
                (Arrays.stream((Long[]) rowResult.get("image"))).filter(x -> {
                    if (x == null)
                        return false;
                    Long z = (Long) x;
                    boolean b = z != 0;
            ...
            // Hack: it seems that Hibernate-type return a Long if 1 element or a List<Long> if
            // more than 1 element.
            ...
            if (image != null) {
                ...
                result.put("macroURL", UrlApi.getAssociatedImage(image, "abstractimage", "macro",
                    (String) result.get("contentType"), 256, "png"));
            } else if (slice != null) {
    ...
    private Long returnElementOrTakeFirstElementIfArray(Object longOrLongArray) {
        ...
        return (longOrLongArray instanceof Long ? (Long) longOrLongArray :
            (Long) ((List) longOrLongArray).get(0));
    }
    ...
    public CommandResponse update(CytomineDomain domain, JsonObject jsonNewData,
        ...
        if (jsonNewData.get("storage") != null && !Objects.equals(jsonNewData.getJSONAttrLong(
            "storage"), ((UploadedFile) domain).getStorage().getId())) {
     ...
     */
    private CommandResponse deleteChild(CytomineDomain domain, Transaction transaction, Task task
        , boolean printMessage) {
    ...
    private void deleteDependentAbstractImage(CytomineDomain domain, Transaction transaction,
                                              Task task) {
        log.info("deleteDependentAbstractImage");

        for (AbstractImage abstractImage :
    ...
    private void deleteDependentAbstractSlice(CytomineDomain domain, Transaction transaction,
                                              Task task) {
        log.info("deleteDependentAbstractSlice");

        for (AbstractSlice abstractSlice :
            abstractSliceRepository.findAllByUploadedFile((UploadedFile) domain)) {
            abstractSliceService.delete(abstractSlice, transaction, task, false);
        }
    }

    private void deleteDependentCompanionFile(CytomineDomain domain, Transaction transaction,
                                              Task task) {
        log.info("deleteDependentCompanionFile");

        for (CompanionFile companionFile :
            companionFileRepository.findAllByUploadedFile((UploadedFile) domain)) {
            companionFileService.delete(companionFile, transaction, task, false);
        }
    }

    private void deleteDependentUploadedFile(CytomineDomain domain, Transaction transaction,
                                             Task task) {
        log.info("deleteDependentUploadedFile");
        UploadedFile uploadedFile = (UploadedFile) domain;
        taskService.updateTask(task, task != null ?
            "Delete " + uploadedFileRepository.countByParent(uploadedFile) + " uploadedFiles " +
                "children" : "");
