...
public class SliceCoordinatesService {
    ...
    public SliceCoordinate getReferenceSliceCoordinate(AbstractImage image) {
        ...
        if (sliceCoordinates.getChannels().isEmpty()) {
            throw new WrongArgumentException("Cannot retrieve reference slices for AbstractImage "
                + image.getId());
        }
    ...
    public AbstractSlice getReferenceSlice(AbstractImage abstractImage) {
        ...
        return abstractSliceRepository.findByImageAndChannelAndZStackAndTime(abstractImage,
                sliceCoordinate.getChannel(), sliceCoordinate.getZStack(),
                sliceCoordinate.getTime())
            .orElseThrow(() -> new ObjectNotFoundException("AbstractSlice",
                "image:" + abstractImage.getId() + "," + sliceCoordinate.getChannel() + ":" + sliceCoordinate.getZStack() + ":" + sliceCoordinate.getTime()));
    }
    ...
    public SliceInstance getReferenceSliceOptimized(ImageInstance imageInstance) {
        ...
        if (sliceCoordinates.getChannels().isEmpty()) {
            throw new WrongArgumentException("Cannot retrieve reference slices for AbstractImage "
                + imageInstance.getBaseImage().getId());
        }
        SliceCoordinate referenceSliceCoordinates = new SliceCoordinate(
            sliceCoordinates.getChannels().get((int) Math.floor(sliceCoordinates.getChannels().size() / 2)),
            sliceCoordinates.getZStacks().get((int) Math.floor(sliceCoordinates.getZStacks().size() / 2)),
        ...
        SliceInstance result =
            slicesInstances.stream().filter(x -> x.getImage().getId().equals(imageInstance.getId()) && x.getBaseSlice().getChannel().equals(referenceSliceCoordinates.getChannel()) && x.getBaseSlice().getZStack().equals(referenceSliceCoordinates.getZStack()) && x.getBaseSlice().getTime().equals(referenceSliceCoordinates.getTime()))
                .findFirst().orElseThrow(() -> new ObjectNotFoundException("AbstractSlice",
                    "image:" + imageInstance.getBaseImage().getId() + "," + referenceSliceCoordinates.getChannel() + ":" + referenceSliceCoordinates.getZStack() + ":" + referenceSliceCoordinates.getTime()));
        return result;
    }
}
