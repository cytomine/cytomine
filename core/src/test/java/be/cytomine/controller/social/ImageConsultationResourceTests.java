...
public class ImageConsultationResourceTests {
    ...
    public void add_consultation() throws Exception {
        ...
        restImageConsultationControllerMockMvc.perform(post("/api/imageinstance/{id}/consultation" +
        ...
        Page<PersistentImageConsultation> persisted =
            persistentImageConsultationRepository.findAllByProjectAndUser(imageInstance.getProject().getId(), user.getId(), PageRequest.of(0, 1));
    ...
    public void list_last_image_of_users_for_a_project() throws Exception {
        ...
        restImageConsultationControllerMockMvc.perform(get("/api/project/{image}/lastImages.json"
    ...
    public void list_last_user_consultation() throws Exception {
        ...
        imageConsultationService.add(user, imageInstance1.getId(), "xxx", "mode",
            DateUtils.addSeconds(new Date(), -3));
        imageConsultationService.add(user, imageInstance1.getId(), "xxx", "mode",
            DateUtils.addSeconds(new Date(), -2));
        imageConsultationService.add(user, imageInstance2.getId(), "xxx", "mode",
            DateUtils.addSeconds(new Date(), -1));


        restImageConsultationControllerMockMvc.perform(get("/api/imageinstance/method/lastopened" +
                ".json")
                .param("max", "10"))
            .andExpect(status().isOk())
    ...
    public void list_last_user_consultation_for_a_project_and_user_distinct_image() throws Exception {
        ...
        restImageConsultationControllerMockMvc.perform(get("/api/project/{project}/user/{user" +
                    "}/imageconsultation.json",
        ...
        restImageConsultationControllerMockMvc.perform(get("/api/project/{project}/user/{user" +
                    "}/imageconsultation.json",
                imageInstance1.getProject().getId().toString(),
                builder.given_a_user().getId().toString())
                .param("distinctImages", "true"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection", hasSize(equalTo(0))));
    }
    ...
    public void resume_by_user_and_project_export_as_csv() throws Exception {
        ...
        MvcResult mvcResult = restImageConsultationControllerMockMvc.perform(get("/api" +
                "/imageconsultation/resume.json")
                .param("project", imageInstance1.getProject().getId().toString())
                .param("user", user.getId().toString())
                .param("export", "csv"))
        ...
        AssertionsForClassTypes.assertThat(imageConsultationResult[6]).isEqualTo(serverUrl +
            "/api/imageinstance/" + imageInstance1.getId() + "/thumb.png?maxSize=512");
    ...
    public void count_annotation_by_project() throws Exception {
        ...
        given_a_persistent_image_consultation(user, imageInstance2, DateUtils.addDays(new Date(),
            -2));

        restImageConsultationControllerMockMvc.perform(get("/api/project/{project" +
                "}/imageconsultation/count.json", imageInstance1.getProject().getId())
