...
public class TaskService {
    ...
    private Task saveOnDatabase(Task task) {
        ...
        if (getFromDatabase(task.getId()) == null) {
            ...
            if (task.getProjectIdent() == null) {
                entityManager.createNativeQuery("INSERT INTO task (id,progress,user_id," +
                        "print_in_activity) VALUES (?,?,?,?)")
                    .setParameter(1, task.getId())
                ...
                entityManager.createNativeQuery("INSERT INTO task (id,progress,project_id," +
                        "user_id,print_in_activity) VALUES (?,?,?,?,?)")
                    .setParameter(1, task.getId())
                    .setParameter(2, task.getProgress())
    ...
    private List<TaskComment> listFromDatabase(Long idProject) {
        List<Object[]> rows = entityManager.createNativeQuery(
                "SELECT comment, timestamp FROM task_comment tc, task t WHERE tc.task_id = t.id  " +
                    "AND project_id = ? and t.print_in_activity=true ORDER BY timestamp DESC")
    ...
    private void saveCommentOnDatabase(TaskComment comment) {
        entityManager.createNativeQuery("INSERT INTO task_comment (task_id,comment,timestamp) " +
                "VALUES (?,?,?)")
