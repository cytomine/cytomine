...
public class WebSocketUserPositionTests {
    ...
    public void add_session_for_already_connected_user() {
        ConcurrentWebSocketSessionDecorator sessionDecorator =
            mock(ConcurrentWebSocketSessionDecorator.class);
    ...
    public void add_track_session_to_not_tracked_user() {
        ...
        WebSocketSession session = mock(WebSocketSession.class);
        when(session.getAttributes()).thenReturn(sessionAttributes(userId, imageInstanceId,
            "false"));
        ...
        ConcurrentWebSocketSessionDecorator createdSession =
    ...
    public void add_some_track_sessions_to_already_tracked_user() {
        ConcurrentWebSocketSessionDecorator followerSession1 =
            mock(ConcurrentWebSocketSessionDecorator.class);
        ConcurrentWebSocketSessionDecorator followerSession2 =
            mock(ConcurrentWebSocketSessionDecorator.class);
        ConcurrentWebSocketSessionDecorator followerSession3 =
            mock(ConcurrentWebSocketSessionDecorator.class);
        ConcurrentWebSocketSessionDecorator broadcastSession =
            mock(ConcurrentWebSocketSessionDecorator.class);
        ...
        WebSocketSession session = mock(WebSocketSession.class);
        when(session.getAttributes()).thenReturn(sessionAttributes(userId2, imageInstanceId,
            "false"));
    ...
    public void remove_tracking_sessions_from_tracked_sessions() throws Exception {
        ...
        // Close the followerSession (by calling afterConnectionClosed with mock session with
        // same session id)
    ...
    public void remove_broadcasting_sessions_from_tracked_sessions() throws Exception {
        ...
        // Close the broadcastSession (by calling afterConnectionClosed with mock session with
        // same session id)
        webSocketUserPositionHandler.afterConnectionClosed(session, CloseStatus.NO_STATUS_CODE);
    ...
    public void update_position_of_tracked_user_send_message_works() throws IOException {
        ...
        assertDoesNotThrow(() -> webSocketUserPositionHandler.sendPositionToFollowers(userId,
            imageInstanceId, "position"));
    ...
    public void remove_session_if_connection_closed() throws Exception {
        ...
        // Close the session (by calling afterConnectionClosed with mock session with same
        // session id)
        webSocketUserPositionHandler.afterConnectionClosed(session, CloseStatus.NO_STATUS_CODE);
    ...
    private void connectSession(WebSocketSession session, String userId, String imageId,
                                String broadcast) {
        when(session.getAttributes()).thenReturn(Map.of("userId", userId, "imageId", imageId,
            "broadcast", broadcast));
        webSocketUserPositionHandler.afterConnectionEstablished(session);
    }

    private void initFollowingSession(String userAndImageId,
                                      ConcurrentWebSocketSessionDecorator broadcastSession,
                                      ConcurrentWebSocketSessionDecorator followerSession) {
        WebSocketUserPositionHandler.sessionsBroadcast.put(userAndImageId, broadcastSession);
        WebSocketUserPositionHandler.sessionsTracked.put(broadcastSession,
