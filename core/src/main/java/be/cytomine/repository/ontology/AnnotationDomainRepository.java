...
public interface AnnotationDomainRepository extends JpaRepository<AnnotationDomain, Long>,
    JpaSpecificationExecutor<AnnotationDomain> {
    ...
    @Query(value = "SELECT annotation.id as annotation,user_id as user\n" +
        ...
        "AND ST_Intersects(annotation.location,ST_GeometryFromText(:location,0))", nativeQuery =
        true)
    ...
    @Query(value = "SELECT count(annotation.id) FROM user_annotation annotation WHERE annotation" +
        ".project_id = :projectId", nativeQuery = true)
    Long countAllUserAnnotationAndProject(Long projectId);

    @Query(value = "SELECT count(annotation.id) FROM reviewed_annotation annotation WHERE " +
        "annotation.project_id = :projectId", nativeQuery = true)
