...
public class RelationTermService extends ModelService {
     ...
     * @param position Term position in relation (term x PARENT term y => term x position 1, term
     *                 y position 2)
    ...
    public List<RelationTerm> list(Term term, String position) {
        ...
        return position.equals("1") ? relationTermRepository.findAllByTerm1(term) :
    ...
    public List<String> getStringParamsI18n(CytomineDomain domain) {
        ...
        return Arrays.asList(String.valueOf(rt.getId()), rt.getRelation().getName(),
            rt.getTerm1().getName(), rt.getTerm2().getName());
    }
    ...
    public void checkDoNotAlreadyExist(CytomineDomain domain) {
        ...
        if (relationTerm != null && relationTerm.getRelation() != null && relationTerm.getTerm2() != null && relationTerm.getTerm2() != null) {
            if (relationTermRepository.findByRelationAndTerm1AndTerm2(relationTerm.getRelation(),
                relationTerm.getTerm1(), relationTerm.getTerm2()).stream().anyMatch(x -> !Objects.equals(x.getId(), relationTerm.getId()))) {
    ...
    public CytomineDomain retrieve(JsonObject json) {
        return relationTermRepository.findByRelationAndTerm1AndTerm2(json.getJSONAttrLong(
                "relation"), json.getJSONAttrLong("term1"), json.getJSONAttrLong("term2"))
