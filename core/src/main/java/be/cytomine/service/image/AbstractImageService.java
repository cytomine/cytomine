...
public class AbstractImageService extends ModelService {
    ...
    public Optional<AbstractImage> findByUploadedFile(Long id) {
        ...
        Optional<AbstractImage> abstractImage =
            abstractImageRepository.findAllByUploadedFile(uploadedFile).stream().findAny();
        abstractImage.ifPresent(image -> securityACLService.check(image.container(), READ));
        return abstractImage;
    }
    ...
    public Page<AbstractImage> list(Project project, List<SearchParameterEntry> searchParameters,
                                    Pageable pageable) {
        List<SearchParameterEntry> validSearchParameters =
            SQLSearchParameter.getDomainAssociatedSearchParameters(AbstractImage.class,
                searchParameters, getEntityManager());

        Specification<AbstractImage> specification =
            SpecificationBuilder.getSpecificationFromFilters(validSearchParameters);
        ...
        if (!currentRoleService.isAdminByNow(currentUserService.getCurrentUser())) {
            ...
            Specification<AbstractImage> filterStorages = (root, query, criteriaBuilder) -> {
                Join<AbstractImage, UploadedFile> uploadedFileJoin =
                    root.join(AbstractImage_.uploadedFile);
        ...
        if (project != null) {
            TreeSet<Long> inProjectImagesId =
                new TreeSet<>(imageInstanceRepository.findAllByProject(project).stream().map(x -> x.getBaseImage().getId())
    ...
    public CommandResponse update(CytomineDomain domain, JsonObject jsonNewData,
        ...
        boolean magnificationUpdated = !Objects.equals(magnification,
            abstractImage.getMagnification());
        boolean physicalSizeXUpdated = !Objects.equals(physicalSizeX,
            abstractImage.getPhysicalSizeX());
        ...
        if (physicalSizeXUpdated && magnificationUpdated) {
            if (physicalSizeX != null && magnification != null) {
                images.addAll(imageInstanceRepository.findAllByBaseImage(abstractImage,
                    x -> physicalSizeX.equals(x.getPhysicalSizeX()) && magnification.equals(x.getMagnification())));
            } else if (physicalSizeX != null) {
                images.addAll(imageInstanceRepository.findAllByBaseImage(abstractImage,
                    x -> physicalSizeX.equals(x.getPhysicalSizeX()) && x.getMagnification() == null));
            } else if (magnification != null) {
                images.addAll(imageInstanceRepository.findAllByBaseImage(abstractImage,
                    x -> magnification.equals(x.getMagnification()) && x.getPhysicalSizeX() == null));
            } else {
                images.addAll(imageInstanceRepository.findAllByBaseImage(abstractImage,
                    x -> x.getMagnification() == null && x.getPhysicalSizeX() == null));
            }
            ...
            if (physicalSizeX != null) {
                images.addAll(imageInstanceRepository.findAllByBaseImage(abstractImage,
                    x -> physicalSizeX.equals(x.getPhysicalSizeX())));
            } else {
                images.addAll(imageInstanceRepository.findAllByBaseImage(abstractImage,
                    x -> x.getPhysicalSizeX() == null));
            }
        ...
        if (magnificationUpdated) {
            if (magnification != null) {
                images.addAll(imageInstanceRepository.findAllByBaseImage(abstractImage,
                    x -> magnification.equals(x.getMagnification())));
            } else {
                images.addAll(imageInstanceRepository.findAllByBaseImage(abstractImage,
                    x -> x.getMagnification() == null));
            }
    ...
    public CommandResponse delete(CytomineDomain domain, Transaction transaction, Task task,
        ...
        if (!isAbstractImageUsed(domain.getId())) {
            ...
            List<ImageInstance> instances =
                imageInstanceRepository.findAllByBaseImage((AbstractImage) domain);
            throw new ForbiddenException("Abstract Image has instances in active projects : " +
                ...
                Map.of("projectNames",
                    instances.stream().map(x -> x.getProject().getName()).collect(Collectors.toList()), "imageNames", instances.stream().map(x -> x.getInstanceFilename()).distinct().collect(Collectors.toList())));
        }
    ...
    public List<Object> getStringParamsI18n(CytomineDomain domain) {
        return List.of(domain.getId(),
            StringUtils.getBlankIfNull(((AbstractImage) domain).getOriginalFilename()));
    }
    ...
    private void deleteDependentAbstractSlice(AbstractImage ai, Transaction transaction,
                                              Task task) {
    ...
    private void deleteDependentImageInstance(AbstractImage ai, Transaction transaction,
                                              Task task) {
        List<ImageInstance> images = imageInstanceRepository.findAllByBaseImage(ai);
        if (!images.isEmpty()) {
            throw new ConstraintException("This image cannot be deleted as it has already been " +
                "insert " +
                "in projects " + images.stream().map(x -> x.getProject().getName()).collect(Collectors.joining(",")));
        }
    }

    private void deleteDependentCompanionFile(AbstractImage ai, Transaction transaction,
                                              Task task) {
    ...
    private void deleteDependentNestedImageInstance(AbstractImage ai, Transaction transaction,
                                                    Task task) {
        List<NestedImageInstance> nestedImageInstances =
            nestedImageInstanceRepository.findAllByBaseImage(ai);
