...
public interface UserAnnotationRepository extends JpaRepository<UserAnnotation, Long>,
    JpaSpecificationExecutor<UserAnnotation> {
    ...
    @Query(
        value = "SELECT a.id id, a.project_id container, '' url FROM user_annotation a, " +
            "image_instance ii, abstract_image ai WHERE a.image_id = ii.id AND ii.base_image_id =" +
            " ai.id AND ai.original_filename not like '%ndpi%svs%' AND GeometryType(a.location) " +
            "!= 'POINT' AND st_area(a.location) < 1500000 ORDER BY st_area(a.location) DESC",
        nativeQuery = true)
    ...
    default List<AnnotationLight> listLight() {
        ...
        for (Tuple tuple : listTuplesLight()) {
            annotationLights.add(new AnnotationLight(
                ...
                UrlApi.getUserAnnotationCropWithAnnotationIdWithMaxSize((Long) tuple.get("id"),
                    256, "png"))
            );
        }
    ...
    @Query(value = "SELECT a.id id, ST_distance(a.location,ST_GeometryFromText(:geometry))  FROM " +
        "user_annotation a WHERE project_id = :projectId", nativeQuery = true)
