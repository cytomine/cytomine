...
public interface ProjectRepository extends JpaRepository<Project, Long> {
    ...
    @Query(value = "select distinct project " +
        "from AclObjectIdentity as aclObjectId, AclEntry as aclEntry, AclSid as aclSid, Project " +
        "as project " +
    ...
    @Query(value = "select id, name from creator_project where user_id = :userId", nativeQuery =
        true)
    ...
    @Query(value = "select id, created as date from project where id NOT IN (:ignoredProjectIds) " +
    ...
    default List<DatedCytomineDomain> listLastCreated() {
        return listLastCreatedTuple().stream().map(x -> new DatedCytomineDomain(((Long) x.get(0))
                , (Date) x.get(1)))
            .collect(Collectors.toList());
    }
    ...
    @Query(value = "SELECT a.project_id FROM image_instance a WHERE id=:imageInstanceId",
        nativeQuery = true)
