...
public interface AclRepository extends JpaRepository<User, Long> {
    ...
    @Query(value =
        "INSERT INTO acl_entry(id,ace_order,acl_object_identity,audit_failure,audit_success,granting,mask,sid) "
            +
            "VALUES(nextval('hibernate_sequence'),:aceOrder,:aclObjectIdentity,false,false,true,:mask,:sid)",
        nativeQuery = true)
    ...
    @Modifying
    @Query(value = "INSERT INTO acl_sid(id,principal,sid) VALUES(nextval('hibernate_sequence'),true,:username)",
        nativeQuery = true)
    ...
    @Modifying
    @Query(value = "INSERT INTO acl_class(id,class) VALUES(nextval('hibernate_sequence'),:className)",
        nativeQuery = true)
