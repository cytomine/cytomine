...
public class SliceCoordinatesService {
    ...
    public AbstractSlice getReferenceSlice(AbstractImage abstractImage) {
        ...
        return abstractSliceRepository.findByImageAndChannelAndZStackAndTime(
                abstractImage,
                sliceCoordinate.getChannel(),
                sliceCoordinate.getZStack(),
    ...
    public SliceInstance getReferenceSliceOptimized(ImageInstance imageInstance) {
        ...
        SliceCoordinates sliceCoordinates = new SliceCoordinates(
            slicesInstances.stream()
                .map(x -> x.getBaseSlice().getChannel())
                .distinct()
                .sorted()
                .collect(Collectors.toList()),
            slicesInstances.stream()
                .map(x -> x.getBaseSlice().getZStack())
                .distinct()
                .sorted()
                .collect(Collectors.toList()),
            slicesInstances.stream()
                .map(x -> x.getBaseSlice().getTime())
                .distinct()
                .sorted()
                .collect(Collectors.toList())
        );
        ...
        SliceInstance result = slicesInstances.stream()
            .filter(x -> x.getImage().getId().equals(imageInstance.getId()) && x.getBaseSlice()
                .getChannel()
                .equals(referenceSliceCoordinates.getChannel()) && x.getBaseSlice()
                .getZStack()
                .equals(referenceSliceCoordinates.getZStack()) && x.getBaseSlice()
                .getTime()
                .equals(referenceSliceCoordinates.getTime()))
            .findFirst()
            .orElseThrow(() -> new ObjectNotFoundException(
                "AbstractSlice",
                "image:"
                    + imageInstance.getBaseImage().getId()
                    + ","
                    + referenceSliceCoordinates.getChannel()
                    + ":"
                    + referenceSliceCoordinates.getZStack()
                    + ":"
                    + referenceSliceCoordinates.getTime()
            ));
        return result;
    }
}
