...
public class JsonObject extends HashMap<String, Object> implements JsonInput {
    ...
    /**
     * Get attr domain value from json Read domain thanks to domain argument class and its id (domain.read) If mandatory
     * flag is true, check if domain exists
     *
     * @return Value as Cytomine Domain
     */
    public CytomineDomain getJSONAttrDomain(
    ...
    /**
     * Get attr domain value from json Read domain thanks to domain argument, get the correct object thanks to value
     * from column (type: columnType) If mandatory flag is true, check if domain exists
     *
     * @return Value as Cytomine Domain
     */
    public CytomineDomain getJSONAttrDomain(
        ...
        if (this.get(attr) != null && !this.get(attr).toString().equals("null")) {
            ...
            if (this.get(attr) instanceof CytomineDomain) {
                ...
                if (domainRead == null) {
                    throw new WrongArgumentException(domain.getClass().getName()
                        + " cannot be found with "
                        + column
                        + " = '"
                        + convertValue(this.get(attr), columnType)
                        + "'");
                }
            }
            if (domainRead == null) {
    ...
    public List<Long> getJSONAttrListLong(String attr) {
        if (this.get(attr) != null && !this.get(attr).toString().equals("null")) {
            if (this.get(attr) instanceof List) {
                return (List<Long>) ((List) this.get(attr)).stream()
                    .map(x -> Long.parseLong(String.valueOf(x)))
                ...
                return Arrays.stream(this.get(attr).toString().split(","))
                    .map(x -> Long.parseLong(String.valueOf(x)))
                    .collect(Collectors.toList());
            }
        }
