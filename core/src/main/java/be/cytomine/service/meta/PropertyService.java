...
public class PropertyService extends ModelService {
    ...
    public Optional<Property> findByDomainAndKey(CytomineDomain domain, String key) {
        Optional<Property> property = propertyRepository.findByDomainIdentAndKey(domain.getId(),
            key);
        if (property.isPresent()) {
            securityACLService.check(property.get().container(), READ);
        }
    ...
    public CommandResponse add(JsonObject jsonObject, Transaction transaction, Task task) {
        ...
        CytomineDomain domain = getCytomineDomain(jsonObject.getJSONAttrStr("domainClassName"),
    ...
    public CommandResponse addProperty(String domainClassName, Long domainIdent, String key,
                                       String value, User user, Transaction transaction) {
    ...
    public CommandResponse update(CytomineDomain domain, JsonObject jsonNewData,
        ...
        User currentUser = currentUserService.getCurrentUser();
        CytomineDomain parentDomain = getCytomineDomain(((Property) domain).getDomainClassName(),
    ...
    public List<String> getStringParamsI18n(CytomineDomain domain) {
        ...
        return Arrays.asList(property.getKey(), property.getDomainClassName(),
            String.valueOf(property.getDomainIdent()));
    }

    public List<Map<String, Object>> listKeysForAnnotation(Project project, ImageInstance image,
                                                           Boolean withUser) {
        if (project != null) {
    ...
    public List<Map<String, Object>> listAnnotationCenterPosition(User user, ImageInstance image,
                                                                  Geometry boundingbox,
                                                                  String key) {
        securityACLService.check(image.container(), READ);
        String request = "SELECT DISTINCT ua.id, ST_X(ST_CENTROID(ua.location)) as x,ST_Y" +
            "(ST_CENTROID(ua.location)) as y, p.value " +
            ...
            (boundingbox != null ?
                "AND ST_Intersects(ua.location,ST_GeometryFromText('" + boundingbox.toString() +
                    "',0)) " : "");
    ...
    private List<Map<String, Object>> selectListKeyWithUser(String request,
        ...
        List<Tuple> resultList = nativeQuery.getResultList();
        return resultList.stream().map(x -> Map.of("key", x.get(0), "user",
            (x.getElements().size() > 1 ? castToLong(x.get(1)) : 0L))).collect(Collectors.toList());
    }
    ...
    private List<Map<String, Object>> selectsql(String request, Map<String, Object> parameters) {
        ...
        List<Tuple> resultList = nativeQuery.getResultList();
        return resultList.stream().map(x -> Map.of("idAnnotation", castToLong(x.get(0)), "x",
            x.get(1), "y", x.get(2), "value", x.get(3))).collect(Collectors.toList());
    }
