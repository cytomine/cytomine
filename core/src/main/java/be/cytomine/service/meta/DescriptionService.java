...
public class DescriptionService extends ModelService {
    ...
    public CytomineDomain retrieve(JsonObject json) {
        ...
        try {
            domain = (CytomineDomain) getEntityManager().find(
                Class.forName(json.getJSONAttrStr("domainClassName")),
                json.getJSONAttrLong("domainIdent")
            );
            ...
            if (description != null) {
                ...
                throw new ObjectNotFoundException("Description not found for domain " + json.getJSONAttrStr(
    ...
    public void checkDoNotAlreadyExist(CytomineDomain domain) {
        Optional<Description>
            description
            = descriptionRepository.findByDomainIdentAndDomainClassName(
            ((Description) domain).getDomainIdent(),
            ((Description) domain).getDomainClassName()
        );
        if (description.isPresent() && !Objects.equals(description.get().getId(), domain.getId())) {
            throw new AlreadyExistException("Description already exists for "
                + ((Description) domain).getDomainClassName()
                + " "
                + ((Description) domain).getDomainIdent());
        }
