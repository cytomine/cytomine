...
### Uploading a task bundle to the app-engine fails with "Unknown bundle archive format"

When building task bundle images with Docker, uploading the generated bundle to the Cytomine app-engine may fail with: `Unknown bundle archive format`.

Docker 29.y.z produces OCI-format images with attestation manifests by default, which appears to be incompatible with the registry-client.

#### Fix / Workaround

Disable provenance and SBOM generation when building the Docker image:

```bash
docker build --provenance=false --sbom=false -t cytomine/example:0.1.0 .
```

Then generate the task bundle as usual:

```bash
docker save cytomine/example:0.1.0 -o org.cytomine.example-0.1.0.tar
zip org.cytomine.example-0.1.0.zip descriptor.yaml org.cytomine.example-0.1.0.tar
```

The resulting bundle should upload successfully.
