...
public class SliceInstanceResourceTests {
    ...
    public void get_an_slice_instance_with_coordinates() throws Exception {
        ...
        restSliceInstanceControllerMockMvc.perform(get(
                ...
                image.getBaseSlice().getChannel(),
                image.getBaseSlice().getZStack(),
                image.getBaseSlice().getTime()
            ))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.id").value(image.getId().intValue()));
    }
    ...
    public void get_slice_instance_thumb() throws Exception {
        ...
        MvcResult mvcResult = restSliceInstanceControllerMockMvc.perform(get(
                "/api/sliceinstance/{id}/thumb.png?maxSize=512",
                image.getId()
            ))
            .andExpect(status().isOk())
            .andReturn();
    ...
    public void channel_histograms_bounds() throws Exception {
        ...
        restSliceInstanceControllerMockMvc.perform(get(
                "/api/sliceinstance/{id}/channelhistogram/bounds.json",
                image.getId()
            ))
            .andExpect(status().isOk())
