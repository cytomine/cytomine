...
public class AbstractSliceResourceTests {
    ...
    public void list_abstract_slice_by_abstract_image() throws Exception {
        ...
        restAbstractSliceControllerMockMvc.perform(get("/api/abstractimage/{id}/abstractslice" +
                ".json", abstractSlice.getImageId()))
    ...
    public void list_abstract_slice_by_uploaded_file() throws Exception {
        ...
        restAbstractSliceControllerMockMvc.perform(get("/api/uploadedfile/{id}/abstractslice" +
                ".json", abstractSlice.getUploadedFile().getId()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection[?(@.id==" + abstractSlice.getId() + ")]").exists());
    }
    ...
    public void get_an_abstract_slice() throws Exception {
        ...
        restAbstractSliceControllerMockMvc.perform(get("/api/abstractslice/{id}.json",
                image.getId()))
            .andExpect(status().isOk())
    ...
    public void get_an_abstract_slice_with_coordinates() throws Exception {
        ...
        restAbstractSliceControllerMockMvc.perform(get("/api/abstractimage/{id}/{channel}/{zStack" +
                    "}/{time}/abstractslice.json",
    ...
    public void edit_valid_abstract_slice() throws Exception {
        ...
        restAbstractSliceControllerMockMvc.perform(put("/api/abstractslice/{id}.json",
                abstractSlice.getId())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    public void delete_abstract_slice() throws Exception {
        AbstractSlice abstractSlice = builder.given_an_abstract_slice();
        restAbstractSliceControllerMockMvc.perform(delete("/api/abstractslice/{id}.json",
                abstractSlice.getId()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.printMessage").value(true))
            .andExpect(jsonPath("$.callback").exists())
    ...
    public void get_abstract_slice_uploader() throws Exception {
        ...
        restAbstractSliceControllerMockMvc.perform(get("/api/abstractslice/{id}/user.json",
                image.getId()))
            .andExpect(status().isOk())
    ...
    public void get_abstract_slice_thumb() throws Exception {
        ...
        MvcResult mvcResult = restAbstractSliceControllerMockMvc.perform(get("/api/abstractslice" +
    ...
    public void get_abstract_slice_tile() throws Exception {
        ...
        stubFor(get(urlEqualTo(IMS_API_BASE_PATH + "/image/" + URLEncoder.encode(image.getPath(),
            ...
            "?channels=0&z_slices=0&timepoints=0&filters=binary"))
            .willReturn(
                aResponse().withBody(mockResponse)
            )
        );
        ...
        MvcResult mvcResult = restAbstractSliceControllerMockMvc.perform(get("/api/abstractslice" +
                    "/{id}/normalized-tile/zoom/{z}/tx/{tx}/ty/{ty}.jpg?filters=binary&channels=0",
                image.getId(), 2, 4, 6))
            .andExpect(status().isOk())
            .andReturn();
        List<LoggedRequest> all = wireMockServer.findAll(RequestPatternBuilder.allRequests());
    ...
    public void get_abstract_slice_tile_if_image_not_exist() throws Exception {
        restAbstractSliceControllerMockMvc.perform(get("/api/abstractslice/{id}/normalized-tile" +
