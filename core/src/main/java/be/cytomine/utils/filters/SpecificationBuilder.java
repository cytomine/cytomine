...
public class SpecificationBuilder {
    ...
    public static <T extends CytomineDomain> Specification<T> getSpecificationFromFilters(
        List<SearchParameterEntry> filters
    ) {
        if (filters.size() > 0) {
    ...
    public static Object castToRequiredType(Class fieldType, Object value) {
        ...
        if (value instanceof List) {
            ...
            return ((List) value).stream()
                .map(x -> castToRequiredType(fieldType, x))
        ...
        throw new WrongArgumentException(
            "Type " + value.getClass().getName() + " not supported for filter " + fieldType
        );
    }
