...
public class AnnotationTermService extends ModelService {
    ...
    public List<AnnotationTerm> listAnnotationTermNotDefinedByUser(UserAnnotation userAnnotation,
                                                                   User user) {
        securityACLService.check(userAnnotation.container(), READ);
    ...
    public CommandResponse add(JsonObject jsonObject) {
        ...
        ).orElseThrow(() -> new ObjectNotFoundException("UserAnnotation",
            jsonObject.getJSONAttrStr("userannotation")));
    ...
    public CommandResponse delete(CytomineDomain domain, Transaction transaction, Task task,
        ...
        securityACLService.check(((AnnotationTerm) domain).getUserAnnotation().getId(),
            UserAnnotation.class, READ);
    ...
    public CommandResponse addAnnotationTerm(Long idUserAnnotation, Long idTerm,
                                             Long idExpectedTerm, Long idUser, User currentUser,
     ...
     */
    public CommandResponse addWithDeletingOldTerm(Long idAnnotation, Long idTerm,
                                                  Boolean fromAllUser) {
        User currentUser = currentUserService.getCurrentUser();
        AnnotationDomain annotation = AnnotationDomain.findAnnotationDomain(getEntityManager(),
                idAnnotation)
        ...
        if (annotation instanceof UserAnnotation) {
            ...
            //Add annotation term
            return addAnnotationTerm(idAnnotation, idTerm, null, currentUser.getId(), currentUser
                , transaction);
    ...
    public List<String> getStringParamsI18n(CytomineDomain domain) {
        ...
        return Arrays.asList(String.valueOf(rt.getId()),
            String.valueOf(rt.getUserAnnotation().getId()), rt.getTerm().getName(),
            (rt.getUser() != null ? rt.getUser().getUsername() : "undefined"));
    }
    ...
    public void checkDoNotAlreadyExist(CytomineDomain domain) {
        ...
        if (annotationTerm != null && annotationTerm.getUserAnnotation() != null && annotationTerm.getTerm() != null && annotationTerm.getUser() != null) {
            Optional<AnnotationTerm> atAlreadyExist =
                annotationTermRepository.findByUserAnnotationAndTermAndUser(annotationTerm.getUserAnnotation(), annotationTerm.getTerm(), annotationTerm.getUser());
            if (atAlreadyExist.isPresent() && !Objects.equals(atAlreadyExist.get().getId(),
                domain.getId())) {
