...
public class AnnotationIndexResourceTests {
    ...
    public void list_user_annotation_property_show() throws Exception {
        ...
        List<AnnotationIndexLightDTO> slicesLight =
            annotationIndexRepository.findAllLightBySliceInstance(slice.getId());
        restAnnotationIndexControllerMockMvc.perform(get("/api/sliceinstance/{id}/annotationindex" +
                ".json", slice.getId()))
            ...
            .andExpect(jsonPath("$.collection[?(@.user==" + me.getId() + ")]" +
                ".countReviewedAnnotation").value(0));
    }
}
