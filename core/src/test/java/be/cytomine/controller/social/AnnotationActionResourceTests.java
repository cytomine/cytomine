...
public class AnnotationActionResourceTests {
    ...
    public void list_last_user_on_image() throws Exception {
        ...
        restUserPositionControllerMockMvc.perform(get("/api/imageinstance/{image" +
                "}/annotation_action.json", annotationDomain.getImage().getId())
                .param("user", user.getId().toString()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection", hasSize(equalTo(1))));

        restUserPositionControllerMockMvc.perform(get("/api/imageinstance/{image" +
                "}/annotation_action.json", builder.given_an_image_instance().getId())
                .param("user", user.getId().toString()))
            .andExpect(status().isOk())
    ...
    public void list_last_user_on_slice() throws Exception {
        ...
        restUserPositionControllerMockMvc.perform(get("/api/sliceinstance/{image" +
                "}/annotation_action.json", annotationDomain.getSlice().getId())
                .param("user", user.getId().toString()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection", hasSize(equalTo(1))));

        restUserPositionControllerMockMvc.perform(get("/api/sliceinstance/{image" +
                "}/annotation_action.json", builder.given_a_slice_instance().getId())
                .param("user", user.getId().toString()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection", hasSize(equalTo(0))));
    }
    ...
    public void count_annotation_by_project() throws Exception {
        User user = builder.given_superadmin();
        AnnotationDomain annotationDomain = builder.given_a_user_annotation();
        given_a_persistent_annotation_action(DateUtils.addDays(new Date(), -2), annotationDomain,
            user, "select");

        restUserPositionControllerMockMvc.perform(get("/api/project/{project}/annotation_action" +
                "/count.json", annotationDomain.getProject().getId())
