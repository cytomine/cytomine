...
public interface ProjectRepository extends JpaRepository<Project, Long> {
    ...
    @Query(value = "select id, created as date from project where id NOT IN (:ignoredProjectIds) order by date desc",
        nativeQuery = true)
    ...
    default List<DatedCytomineDomain> listLastCreated(List<Long> ignoredProjectIds) {
        return listLastCreatedTuple(ignoredProjectIds).stream()
            .map(x -> new DatedCytomineDomain(((Long) x.get(0)), (Date) x.get(1)))
            .collect(Collectors.toList());
    }
