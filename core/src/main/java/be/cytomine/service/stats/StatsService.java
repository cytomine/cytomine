...
public class StatsService {
    ...
    public List<JsonObject> statAnnotationTermedByProject(Term term) {
        ...
        for (Project project : projects) {
            ...
            List<Long> layers =
                userService.listLayers(project, null).stream().map(x -> x.getJSONAttrLong("id"))
        ...
        return counts.entrySet().stream().map(x -> JsonObject.of("key", x.getKey(), "value",
                x.getValue()))
            .collect(Collectors.toList());

    }

    public List<JsonObject> statAnnotationEvolution(Project project, Term term, int daysRange,
                                                    Date startDate, Date endDate,
                                                    boolean reverseOrder, boolean accumulate) {
        securityACLService.check(project, READ);
        String request = "SELECT created " +
            "FROM UserAnnotation " +
            "WHERE project.id = " + project.getId() + " " +
            (term != null ? "AND id IN (SELECT userAnnotation.id FROM AnnotationTerm WHERE term" +
                ".id = " + term.getId() + ") " : "") +
        ...
        List<Date> annotationsDates =
            entityManager.createQuery(request, Date.class).getResultList();

        List<JsonObject> data = aggregateByPeriods(annotationsDates, daysRange,
            (startDate == null ? project.getCreated() : startDate), (endDate == null ?
                new Date() : endDate), accumulate);
        if (reverseOrder) {
    ...
    public List<JsonObject> statReviewedAnnotationEvolution(Project project, Term term,
                                                            int daysRange, Date startDate,
                                                            Date endDate, boolean reverseOrder,
                                                            boolean accumulate) {
        securityACLService.check(project, READ);
        ...
        String request = "SELECT created " +
            ...
            (term != null ? "AND id IN (SELECT reviewed_annotation_terms_id FROM " +
                "reviewed_annotation_term WHERE term_id = " + term.getId() + ") " : "") +
            (startDate != null ? "AND created > '" + startDate + "'" : "") +
            (endDate != null ? "AND created < '" + endDate + "'" : "") +
            "ORDER BY created ASC";

        List<java.util.Date> annotationsDates =
            entityManager.createNativeQuery(request).getResultList();
...
//        Session session = ((Session) entityManager.getDelegate());
        // this is a new implementation using JakartaEE Criteria API instead of Hibernate
        // Criteria API
        ...
        if (startDate != null) {
            Predicate startDatePredicate = cb.greaterThan(userAnnotationRoot.get("created"),
                startDate);
...
//        List<Object[]> numberOfAnnotatedImagesByUser = criteria.list();
    ...
    public List<JsonObject> statTerm(Project project, Date startDate, Date endDate,
                                     boolean leafsOnly) {
        securityACLService.check(project, READ);
        ...
        for (Tuple row : rows) {
            if (row.get(0) == null) {
                ...
                if (name != null) {
                    stats.put(name, (row.get(1) instanceof BigInteger ? (Long) row.get(1) :
                        row.get(1)));
                }
            }
        }
        ...
        for (Map.Entry<String, Object> entry : stats.entrySet()) {
            list.add(JsonObject.of("id", ids.get(entry.getKey()), "key", (entry.getKey().equals(
                    "0") ? null : entry.getKey()), "value", entry.getValue(), "color",
    ...
    public List<JsonObject> statUserAnnotations(Project project) {
        ...
        cq.multiselect(userAnnotationTermRoot.get("user").get("id"),
            ...
            .where(predicates)
            .groupBy(userAnnotationTermRoot.get("user").get("id"), userAnnotationTermRoot.get(
                "term").get("id"));
        TypedQuery<Tuple> q = entityManager.createQuery(cq);
...
//        Predicate projectPredicate = cb.equal(userAnnotationRoot.get("project"), project);
        ...
        cq.multiselect(userAnnotationRoot.get("user").get("id"),
                cb.countDistinct(userAnnotationRoot.get("id")))
            .where(predicatesList.toArray(Predicate[]::new))
            .groupBy(userAnnotationRoot.get("user").get("id"));
...
//            criteria.add(Restrictions.lt("created", endDate));
//        }
//        criteria.setFetchMode("user", FetchMode.JOIN); //right join possible ? it will be
//        sufficient
...
//        List<Object[]> userAnnotations = criteria.list();
    ...
    public JsonObject statUsedStorage() {
        ...
        return JsonObject.of("total", total, "available", available, "used", used, "usedP",
            percentage);
    }

    public List<JsonObject> statConnectionsEvolution(Project project, int daysRange,
                                                     Date startDate, Date endDate,
                                                     boolean accumulate) {
        securityACLService.check(project, READ);
        ...
        List<PersistentProjectConnection> persistentProjectConnections = mongoTemplate.find(query
            , PersistentProjectConnection.class);
        List<Date> createdDates =
            persistentProjectConnections.stream().map(x -> x.getCreated()).collect(Collectors.toList());
        return this.aggregateByPeriods(createdDates, daysRange, (startDate == null ?
                project.getCreated() : startDate), (endDate == null ? new Date() : endDate),
            accumulate);
    }


    public List<JsonObject> statImageConsultationsEvolution(Project project, int daysRange,
                                                            Date startDate, Date endDate,
                                                            boolean accumulate) {
        securityACLService.check(project, READ);
        Query query = new Query();
        ...
        List<PersistentImageConsultation> persistentProjectConnections = mongoTemplate.find(query
    ...
    public List<JsonObject> statAnnotationActionsEvolution(Project project, int daysRange,
                                                           Date startDate, Date endDate,
        ...
        List<AnnotationAction> persistentProjectConnections = mongoTemplate.find(query,
    ...
    private List<JsonObject> aggregateByPeriods(List<Date> creationDates, int daysRange,
                                                Date startDate, Date endDate, boolean accumulate) {
