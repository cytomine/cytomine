...
public class AnnotationTrackService extends ModelService {
    ...
    public Optional<AnnotationTrack> find(AnnotationDomain annotation, Track track) {
        Optional<AnnotationTrack> optionalAnnotationTrack = annotationTrackRepository.findByAnnotationIdentAndTrack(
            annotation.getId(),
            track
        );
        optionalAnnotationTrack.ifPresent(annotationTrack -> securityACLService.check(
    ...
    public List<String> getStringParamsI18n(CytomineDomain domain) {
        ...
        return Arrays.asList(
            String.valueOf(annotationTrack.getId()),
    ...
    public void checkDoNotAlreadyExist(CytomineDomain domain) {
        ...
        if (annotationTrack != null) {
            if (annotationTrackRepository.findByAnnotationIdentAndTrack(
                annotationTrack.getId(),
                annotationTrack.getTrack()
            ).stream().anyMatch(x -> !Objects.equals(x.getId(), annotationTrack.getId()))) {
                throw new AlreadyExistException("AnnotationTrack already exists for annotation "
                    + annotationTrack.getId()
                    + " and track "
                    + annotationTrack.getTrack().getName());
            }
            if (annotationTrackRepository.findBySliceAndTrack(annotationTrack.getSlice(), annotationTrack.getTrack())
                .stream()
                .anyMatch(x -> !Objects.equals(x.getId(), annotationTrack.getId()))) {
                throw new AlreadyExistException("AnnotationTrack already exists for slice " + annotationTrack.getSlice()
                    .getId() + " and track " + annotationTrack.getTrack().getName());
            }
        }
    }
    ...
    public CytomineDomain retrieve(JsonObject json) {
        ...
        if (json.containsKey("id") && !json.get("id").toString().equals("null")) {
            domain = (CytomineDomain) getEntityManager().find(
                currentDomain(),
                super.retrieveLongId(json)
            ); // cast to long = issue ? TODO
            ...
            domain = annotationTrackRepository.findByAnnotationIdentAndTrack(
