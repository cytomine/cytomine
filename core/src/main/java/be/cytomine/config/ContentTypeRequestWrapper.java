 ...
 * It seems that by default, spring receives 'application/octet-stream' which is not compatible
 * to automatically parse JSON data.
 * So if the request has a ".json" string in its path AND if the header is null or equal to
 * application/octet-stream, then we return a application/json contenttype
 */
    ...
    public String getHeader(String name) {
        try {
            if (name.equalsIgnoreCase("content-type")
                && (super.getHeader(name) == null || super.getHeader(name).equalsIgnoreCase(
