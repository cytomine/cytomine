...
public class CompanionFileResourceTests {
    ...
    public void list_companion_file_by_abstract_image() throws Exception {
        ...
        restCompanionFileControllerMockMvc.perform(get("/api/abstractimage/{id}/companionfile" +
                ".json", companionFile.getImage().getId()))
    ...
    public void list_companion_file_by_uploaded_file() throws Exception {
        ...
        restCompanionFileControllerMockMvc.perform(get("/api/uploadedfile/{id}/companionfile" +
                ".json", companionFile.getUploadedFile().getId()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection", hasSize(greaterThanOrEqualTo(1))))
            .andExpect(jsonPath("$.collection[?(@.id==" + companionFile.getId() + ")]").exists());
    ...
    public void get_an_companion_file() throws Exception {
        ...
        restCompanionFileControllerMockMvc.perform(get("/api/companionfile/{id}.json",
                image.getId()))
            .andExpect(status().isOk())
    ...
    public void add_valid_companion_file() throws Exception {
        CompanionFile companionFile =
            builder.given_a_not_persisted_companion_file(builder.given_an_abstract_image());
    ...
    public void edit_valid_companion_file() throws Exception {
        ...
        restCompanionFileControllerMockMvc.perform(put("/api/companionfile/{id}.json",
                companionFile.getId())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    public void delete_companion_file() throws Exception {
        CompanionFile companionFile =
            builder.given_a_companion_file(builder.given_an_abstract_image());
        restCompanionFileControllerMockMvc.perform(delete("/api/companionfile/{id}.json",
                companionFile.getId()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.printMessage").value(true))
            .andExpect(jsonPath("$.callback").exists())
    ...
    public void get_companion_file_uploader() throws Exception {
        ...
        restCompanionFileControllerMockMvc.perform(get("/api/companionfile/{id}/user.json",
                companionFile.getId()))
            .andExpect(status().isOk())
    ...
    public void download_companion_file() throws Exception {
        ...
        MvcResult mvcResult = restCompanionFileControllerMockMvc.perform(get("/api/companionfile" +
                "/{id}/download", companionFile.getId()))
            .andExpect(status().isOk())
            .andReturn();
