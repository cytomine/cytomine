...
public class TermResourceTests {
    ...
    public void list_all_terms() throws Exception {
        ...
        when(termHttpContract.findAll(any(Pageable.class))).thenReturn(new PageImpl<>(List.of(toTermResponse(term))));
    ...
    public void add_valid_term() throws Exception {
        ...
        restTermControllerMockMvc.perform(
            ...
            .andExpect(jsonPath("$.callback").exists()).andExpect(jsonPath("$.callback.termID").exists())
            ...
            .andExpect(jsonPath("$.term.id").exists()).andExpect(jsonPath("$.term.name").value(term.getName()))
    ...
    public void add_valid_term_by_group() throws Exception {
        ...
        when(termHttpContract.update(any())).thenReturn(toTermResponse(term1));

        restTermControllerMockMvc.perform(post("/api/term.json").contentType(MediaType.APPLICATION_JSON)
    ...
    public void add_term_refused_if_already_exists() throws Exception {
        ...
        when(termHttpContract.update(any())).thenReturn(toTermResponse(term));
    ...
    public void edit_valid_term() throws Exception {
        Term term = builder.given_a_term();
        when(termHttpContract.update(eq(term.getId()), any())).thenReturn(toTermResponse(term));

        restTermControllerMockMvc.perform(
                put("/api/term/{id}.json", term.getId()).contentType(MediaType.APPLICATION_JSON).content(term.toJSON()))
            .andExpect(status().isOk()).andExpect(jsonPath("$.printMessage").value(true))
            .andExpect(jsonPath("$.callback").exists()).andExpect(jsonPath("$.callback.termID").exists())
    ...
    public void delete_term() throws Exception {
        ...
        HttpCommandResponse<TermResponse> response =
            new HttpCommandResponse<>("", new Callback("be.cytomine.DeleteTermCommand", term.getId(),term.getOntology().getId()), true,
                toTermResponse(term), -1);
        when(termHttpContract.delete(eq(term.getId()), anyLong())).thenReturn(Optional.of(response));

        restTermControllerMockMvc.perform(
                delete("/api/term/{id}.json", term.getId()).contentType(MediaType.APPLICATION_JSON).content(term.toJSON()))
            .andExpect(status().isOk()).andExpect(jsonPath("$.printMessage").value(true))
            .andExpect(jsonPath("$.callback").exists()).andExpect(jsonPath("$.callback.termID").exists())
            .andExpect(jsonPath("$.callback.method").value("be.cytomine.DeleteTermCommand"))
            .andExpect(jsonPath("$.callback.ontologyID").value(String.valueOf(term.getOntology().getId())))
            .andExpect(jsonPath("$.message").exists()).andExpect(jsonPath("$.command").exists())
            .andExpect(jsonPath("$.data.id").exists()).andExpect(jsonPath("$.data.name").value(term.getName()))
