        ...
        try {
            ...
            if (!geom.isValid()) {
                ...
                if (!geom.isValid() || geom.isEmpty()) {
                    ...
                    String request =
                        "SELECT ST_AsText(ST_MakeValid(ST_AsText('" + backupLocation + "')))";
                    log.info(request);
                    Query nativeQuery = entityManager.createNativeQuery("SELECT ST_AsText" +
                        "(ST_MakeValid(ST_AsText('" + backupLocation + "')))", Tuple.class);
            ...
            if (type.equals("MULTILINESTRING") || type.equals("GEOMETRYCOLLECTION")) {
                ...
                throw new WrongArgumentException(geom.getGeometryType() + " is not a valid " +
                    "geometry type!");
            }
