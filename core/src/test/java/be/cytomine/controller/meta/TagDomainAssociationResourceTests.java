...
public class TagDomainAssociationResourceTests {
    ...
    public void get_a_tag_domain_association() throws Exception {
        ...
        restTagDomainAssociationControllerMockMvc.perform(get("/api/tag_domain_association/{id}" +
                ".json", tagDomainAssociation.getId()))
    ...
    public void list_all_tag_domain_association_with_filters() throws Exception {
        TagDomainAssociation tagDomainAssociation =
            builder.given_a_tag_association(builder.given_a_tag(), builder.given_a_project());
        TagDomainAssociation tagDomainAssociationWithAnotherTagAndAnnotherDomain =
            builder.given_a_tag_association(builder.given_a_tag(), builder.given_a_project());
        restTagDomainAssociationControllerMockMvc.perform(get("/api/tag_domain_association.json")
        ...
        restTagDomainAssociationControllerMockMvc.perform(get("/api/tag_domain_association.json")
                .param("tag[in]",
                    tagDomainAssociation.getTag().getId().toString() + "," + tagDomainAssociationWithAnotherTagAndAnnotherDomain.getTag().getId().toString()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection", hasSize(greaterThan(0))))
            .andExpect(jsonPath("$.collection[?(@.tag=='" + tagDomainAssociation.getTag().getId() + "')]").exists())
    ...
    public void list_tag_domain_associations_by_domain() throws Exception {
        TagDomainAssociation tagDomainAssociation =
            builder.given_a_tag_association(builder.given_a_tag(), builder.given_a_project());
        restTagDomainAssociationControllerMockMvc.perform(get("/api/domain/{domainClassName" +
                    "}/{domainIdent}/tag_domain_association.json",
                tagDomainAssociation.getDomainClassName(), tagDomainAssociation.getDomainIdent()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection", hasSize(greaterThan(0))))
    ...
    public void list_tag_domain_associations_by_domain_does_not_exists() throws Exception {
        ...
        restTagDomainAssociationControllerMockMvc.perform(get("/api/domain/{domainClassName" +
                    "}/{domainIdent}/tag_domain_association.json",
                tagDomainAssociation.getDomainClassName(), 0))
            .andExpect(status().isNotFound());
    }
    ...
    public void add_valid_association() throws Exception {
        TagDomainAssociation tagDomainAssociation =
            builder.given_a_not_persisted_tag_association(builder.given_a_tag(),
                builder.given_a_project());
        restTagDomainAssociationControllerMockMvc.perform(post("/api/domain/{domainClassName" +
                    "}/{domainIdent}/tag_domain_association.json",
                tagDomainAssociation.getDomainClassName(), tagDomainAssociation.getDomainIdent())
    ...
    public void delete_tag_domain_association() throws Exception {
        ...
        restTagDomainAssociationControllerMockMvc.perform(delete("/api/tag_domain_association/{id" +
                "}.json", tagDomainAssociation.getId())
                .contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk());
    }
