...
public class KmeansGeometryService {
    ...
    public static final Map<Integer, Map<Integer, Integer>> rules =
        Map.of(
            100, Map.of((int) ANNOTATIONSIZE1, FULL, (int) ANNOTATIONSIZE2, FULL,
                (int) ANNOTATIONSIZE3, KMEANSFULL, (int) ANNOTATIONSIZE4, KMEANSFULL,
                (int) ANNOTATIONSIZE5, KMEANSFULL),
            75, Map.of((int) ANNOTATIONSIZE1, FULL, (int) ANNOTATIONSIZE2, FULL,
                (int) ANNOTATIONSIZE3, FULL, (int) ANNOTATIONSIZE4, KMEANSFULL,
                (int) ANNOTATIONSIZE5, KMEANSFULL),
            50, Map.of((int) ANNOTATIONSIZE1, FULL, (int) ANNOTATIONSIZE2, FULL,
                (int) ANNOTATIONSIZE3, FULL, (int) ANNOTATIONSIZE4, FULL, (int) ANNOTATIONSIZE5,
                KMEANSFULL),
            25, Map.of((int) ANNOTATIONSIZE1, FULL, (int) ANNOTATIONSIZE2, FULL,
                (int) ANNOTATIONSIZE3, FULL, (int) ANNOTATIONSIZE4, FULL, (int) ANNOTATIONSIZE5,
                FULL),
            0, Map.of((int) ANNOTATIONSIZE1, FULL, (int) ANNOTATIONSIZE2, FULL,
                (int) ANNOTATIONSIZE3, FULL, (int) ANNOTATIONSIZE4, FULL, (int) ANNOTATIONSIZE5,
                FULL)
    ...
    public List<Kmeans> doKeamsFullRequest(String request) {
        String requestKmeans = "SELECT kmeans, count(*), st_astext(ST_ConvexHull(ST_Collect" +
            "(location))) \n" +
            "FROM (\n" + request + "\n" + ") AS ksub\n" +
            "GROUP BY kmeans\n" +
            "ORDER BY kmeans;";
    ...
    public List<Kmeans> doKeamsSoftRequest(String request) {
        String requestKmeans = "SELECT kmeans, count(*), st_astext(ST_Centroid(ST_Collect" +
            "(location))) \n" +
            "FROM (\n" + request + "\n" + ") AS ksub\n" +
            "GROUP BY kmeans\n" +
            "ORDER BY kmeans;";
    ...
    public int mustBeReduce(List<Long> slices, Long userId, String bbox) {
        ...
        try {
            ...
            throw new WrongArgumentException("Annotation location cannot be converted to " +
                "geometry: " + bbox);
        }
    }
    ...
    public int mustBeReduce(List<SliceInstance> slices, User user, Geometry bbox) {
        ...
        if (images.size() != 1) {
            throw new WrongArgumentException("To use kmeans, all slices must belong to the same " +
                "image.");
        }
