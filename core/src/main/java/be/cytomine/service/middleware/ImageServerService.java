...
public class ImageServerService {
    ...
    public ResponseEntity<byte[]> download(UploadedFile uploadedFile,
        ...
        request.getHeaders().add(org.springframework.http.HttpHeaders.CONTENT_TYPE,
            MediaType.APPLICATION_JSON_VALUE);

        return request.toResponseEntity(proxy, byte[].class);
    }

    public ResponseEntity<byte[]> download(AbstractImage abstractImage,
                                           ProxyExchange<byte[]> proxy) throws IOException {
        PreparedRequest request = new PreparedRequest();
        request.setMethod(HttpMethod.GET);
        request.setUrl(this.internalImageServerURL());
    ...
    public ResponseEntity<byte[]> download(CompanionFile companionFile,
                                           ProxyExchange<byte[]> proxy) throws IOException {
    ...
    public ResponseEntity<byte[]> label(AbstractImage image, LabelParameter params, String etag,
        ...
        request.getHeaders().add(org.springframework.http.HttpHeaders.CONTENT_TYPE,
            MediaType.APPLICATION_JSON_VALUE);
        request.getHeaders().add(org.springframework.http.HttpHeaders.ACCEPT,
    ...
    public ResponseEntity<byte[]> normalizedTile(AbstractSlice slice, TileParameters params,
        ...
        request.addQueryParameter("z_slices", params.getZSlices() != null ? params.getZSlices() :
            slice.getZStack());
        request.addQueryParameter("timepoints", params.getTimepoints() != null ?
            params.getTimepoints() : slice.getTime());
    ...
    public ResponseEntity<byte[]> crop(AnnotationDomain annotation, CropParameter params,
                                       String etag, ProxyExchange<byte[]> proxy) throws UnsupportedEncodingException, ParseException {
    ...
    public ResponseEntity<byte[]> crop(SliceInstance slice, CropParameter params, String etag,
                                       ProxyExchange<byte[]> proxy) throws UnsupportedEncodingException, ParseException {
    ...
    public ResponseEntity<byte[]> crop(AbstractSlice slice, CropParameter params, String etag,
        ...
        if (params.getDraw() != null && params.getDraw()) {
            ...
            body.put("background_transparency", params.getAlpha() != null ? params.getAlpha() :
                100);
        } else {
    ...
    public ResponseEntity<byte[]> window(AbstractSlice slice, WindowParameter params, String etag
        ...
        if (params.getGeometries() != null) {
            String strokeColor = params.getColor() != null ?
                params.getColor().replace("0x", "#") : "black";
            ...
            for (Map<String, Object> geometry : geometries) {
                ...
                if (params.getComplete() != null && params.getComplete() && geometry != null) {
                    wkt =
                        simplifyGeometryService.reduceGeometryPrecision((Geometry) geometry).toText();
                } else if (geometry != null) {
                    wkt =
                        simplifyGeometryService.simplifyPolygonForCrop((Geometry) geometry).toText();
                }
                Map<String, Object> annot = new LinkedHashMap<>(wkt != null ? Map.of("geometry",
                    wkt) : Map.of());
            ...
            switch (annotationType) {
                ...
                case "alphaMask", "alphamask" -> {
                    ...
                    annotationStyle.put("background_transparency", params.getAlpha() != null ?
                        params.getAlpha() : 100);
                }
                default -> {
                    annotationStyle.put("mode", "CROP");
