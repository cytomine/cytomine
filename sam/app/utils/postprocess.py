...
def post_process_segmentation_mask(
    mask: np.ndarray,
    opening_kernel_size: int = 10,
    closing_kernel_size: int = 20,
    blur_size: int = 21,
    ...
    if do_gaussian_blur:
        _, temp_mask = cv2.threshold(
            cv2.GaussianBlur(temp_mask, (blur_size, blur_size), 0),
            127,
            255,
            cv2.THRESH_BINARY,
        )
...
def filter_mask_by_size(
    mask: np.ndarray,
    area_thresh_percentage: float = 0.1,
) -> np.ndarray:
    """
    Filters the mask by area, it finds the area of the largest component of the mask
    and removes all the components that are inferior to a certain threshold (%) of
    that area, it enables to remove small noises in the mask.

    Args:
        (mask: np.ndarray): the mask to filter.
        (area_thresh_percentage: float): the % of the max component under which it
