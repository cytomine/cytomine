...
public class AttachedFileResourceTests {
    ...
    public void list_attached_files_by_domain() throws Exception {
        AttachedFile attachedFile = builder.given_a_attached_file(builder.given_a_project());
        restTermControllerMockMvc.perform(get("/api/domain/{domainClassName}/{domainIdent" +
                    "}/attachedfile.json", attachedFile.getDomainClassName(),
    ...
    public void list_attached_files_by_domain_does_not_exists() throws Exception {
        AttachedFile attachedFile = builder.given_a_attached_file(builder.given_a_project());
        restTermControllerMockMvc.perform(get("/api/domain/{domainClassName}/{domainIdent" +
                "}/attachedfile.json", attachedFile.getDomainClassName(), 0))
            .andExpect(status().isNotFound());
    }
    ...
    public void download_attached_file() throws Exception {
        ...
        MvcResult mvcResult = restTermControllerMockMvc.perform(get("/api/attachedfile/{id" +
                "}/download", attachedFile.getId()))
    ...
    public void upload_attached_file() throws Exception {
        ...
        AttachedFile attachedFile =
            attachedFileRepository.findAllByDomainClassNameAndDomainIdent(project.getClass().getName(), project.getId()).get(0);
    ...
    public void delete_attached_file() throws Exception {
        ...
        restTermControllerMockMvc.perform(delete("/api/attachedfile/{id}.json",
                attachedFile.getId())
