...
public class AbstractSliceResourceTests {
    ...
    public void list_abstract_slice_by_uploaded_file() throws Exception {
        ...
        restAbstractSliceControllerMockMvc.perform(get(
                ...
                abstractSlice.getUploadedFile().getId()
            ))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection[?(@.id==" + abstractSlice.getId() + ")]").exists());
    }
    ...
    public void get_abstract_slice_thumb() throws Exception {
        ...
        MvcResult mvcResult = restAbstractSliceControllerMockMvc.perform(get(
                "/api/abstractslice/{id}/thumb.png?maxSize=512",
                image.getId()
            ))
            .andExpect(status().isOk())
            .andReturn();
    ...
    public void get_abstract_slice_tile() throws Exception {
        ...
        MvcResult mvcResult = restAbstractSliceControllerMockMvc.perform(get(
                "/api/abstractslice/{id}/normalized-tile/zoom/{z}/tx/{tx}/ty/{ty}.jpg?filters=binary&channels=0",
                image.getId(),
                2,
                4,
                6
    ...
    public void get_abstract_slice_window() throws Exception {
        ...
        MvcResult mvcResult = restAbstractSliceControllerMockMvc.perform(get(
                "/api/abstractslice/{id}/window-10-20-30-40.png",
                image.getId()
            ))
            .andExpect(status().isOk())
            .andReturn();
