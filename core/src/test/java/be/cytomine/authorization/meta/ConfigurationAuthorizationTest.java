...
public class ConfigurationAuthorizationTest extends AbstractAuthorizationTest {
    ...
    public void admin_read_config() {
        expectOK(() -> {
            configurationService.findByKey(configForAdmin.getKey());
        });
        expectOK(() -> {
            configurationService.findByKey(configForUser.getKey());
        });
        expectOK(() -> {
            configurationService.findByKey(configForAll.getKey());
        });
    }
    ...
    public void admin_can_create_config() {
        expectOK(() -> {
            configurationService.add(configForUser.toJsonObject().withChange("id", null).withChange("key", UUID.randomUUID().toString()));
        });
    }
    ...
    public void admin_can_edit_config() {
        expectOK(() -> {
            configurationService.update(configForUser, configForUser.toJsonObject().withChange(
                "value", "newvalue"));
        });
    }
    ...
    public void admin_can_delete_config() {
        ...
        expectOK(() -> {
            configurationService.delete(configuration, null, null, false);
        });
    }
