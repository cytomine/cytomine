...
public class AnnotationTrackResourceTests {
    ...
    public void list_all_annotationTracks_by_track() throws Exception {
        ...
        restAnnotationTrackControllerMockMvc.perform(get("/api/track/{id}/annotationtrack.json",
    ...
    public void list_all_annotationTracks_by_annotation() throws Exception {
        AnnotationTrack annotationTrack = builder.given_a_annotation_track();
        restAnnotationTrackControllerMockMvc.perform(get("/api/annotation/{id}/annotationtrack" +
                ".json", annotationTrack.getAnnotationIdent()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection", hasSize(greaterThan(0))));
    }
    ...
    public void get_a_annotationTrack() throws Exception {
        AnnotationTrack annotationTrack = builder.given_a_annotation_track();
        restAnnotationTrackControllerMockMvc.perform(get("/api/annotationtrack/{annotation" +
                    "}/{track}.json", annotationTrack.getAnnotationIdent(),
                annotationTrack.getTrack().getId()))
            .andExpect(status().isOk())
    ...
    public void get_a_annotationTrack_annotation_not_exists() throws Exception {
        AnnotationTrack annotationTrack = builder.given_a_annotation_track();
        restAnnotationTrackControllerMockMvc.perform(get("/api/annotationtrack/{annotation" +
                "}/{track}.json", 0, annotationTrack.getTrack().getId()))
            .andExpect(status().isNotFound());
    }
    ...
    public void get_a_annotationTrack_track_not_exists() throws Exception {
        AnnotationTrack annotationTrack = builder.given_a_annotation_track();
        restAnnotationTrackControllerMockMvc.perform(get("/api/annotationtrack/{annotation" +
                "}/{track}.json", annotationTrack.getAnnotationIdent(), 0))
            .andExpect(status().isNotFound());
    }
    ...
    public void delete_annotationTrack() throws Exception {
        AnnotationTrack annotationTrack = builder.given_a_annotation_track();
        restAnnotationTrackControllerMockMvc.perform(delete("/api/annotationtrack/{annotation" +
                    "}/{track}.json", annotationTrack.getAnnotationIdent(),
                annotationTrack.getTrack().getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(annotationTrack.toJSON()))
