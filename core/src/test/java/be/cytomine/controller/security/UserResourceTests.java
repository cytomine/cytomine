...
public class UserResourceTests {
    ...
    public void list_project_admin() throws Exception {
        ...
        restUserControllerMockMvc.perform(get("/api/project/{id}/admin.json", project.getId()))
            ...
            .andExpect(jsonPath("$.collection[?(@.username=='" + projectAdmin.getUsername() + "')" +
                "]").exists())
            .andExpect(jsonPath("$.collection[?(@.username=='" + projectUser.getUsername() + "')" +
                "]").doesNotExist());
    ...
    public void list_project_representatives() throws Exception {
        ...
        restUserControllerMockMvc.perform(get("/api/project/{id}/users/representative.json",
                project.getId()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection", hasSize(greaterThan(0))))
    ...
    public void list_project_creator() throws Exception {
        ...
        restUserControllerMockMvc.perform(get("/api/project/{id}/users/representative.json",
            ...
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection", hasSize(greaterThan(0))))
            .andExpect(jsonPath("$.collection[?(@.username=='" + projectCreator.getUsername() +
                "')]").exists())
            .andExpect(jsonPath("$.collection[?(@.username=='" + projectUser.getUsername() + "')" +
                "]").doesNotExist());
    }
    ...
    public void list_storage_user() throws Exception {
        ...
        restUserControllerMockMvc.perform(get("/api/storage/{id}/user.json", storage.getId()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection", hasSize(greaterThan(0))))
            .andExpect(jsonPath("$.collection[?(@.username=='" + storageAdmin.getUsername() + "')" +
                "]").exists())
            .andExpect(jsonPath("$.collection[?(@.username=='" + storageUser.getUsername() + "')" +
                "]").exists())
            .andExpect(jsonPath("$.collection[?(@.username=='" + simpleUser.getUsername() + "')]").doesNotExist());
    }
    ...
    public void get_user_as_superadmin() throws Exception {
        ...
        restUserControllerMockMvc.perform(get("/api/user/{id}.json",
    ...
    public void get_user_with_its_username() throws Exception {
        User currentUser = builder.given_superadmin();
        restUserControllerMockMvc.perform(get("/api/user/{id}.json",
                builder.given_superadmin().getUsername())
            )
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.username").value(currentUser.getUsername()));
    }
    ...
    public void get_user_with_its_user_key() throws Exception {
        User currentUser = builder.given_superadmin();
        restUserControllerMockMvc.perform(get("/api/userkey/{publicKey}/keys.json",
                builder.given_superadmin().getPublicKey())
            )
            .andExpect(status().isOk())
    ...
    public void get_user_with_its_user_key_id() throws Exception {
        User currentUser = builder.given_superadmin();
        restUserControllerMockMvc.perform(get("/api/user/{id}/keys.json",
                builder.given_superadmin().getId())
            )
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.privateKey").value(currentUser.getPrivateKey()));
...
//    }
    ...
    public void add_user_to_project() throws Exception {
        ...
        Project project = builder.given_a_project();
        User user = builder.given_a_user();
        restUserControllerMockMvc.perform(post("/api/project/{project}/user/{user}.json",
    ...
    public void delete_user_from_project() throws Exception {
        ...
        restUserControllerMockMvc.perform(delete("/api/project/{project}/user/{user}.json",
                project.getId(), user.getId())
                .contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk());
    ...
    public void delete_users_from_project() throws Exception {
        ...
        restUserControllerMockMvc.perform(delete("/api/project/{project}/user.json",
                project.getId())
                .param("users", user1.getId() + "," + user2.getId())
                .contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk());
    ...
    public void add_admin_to_project() throws Exception {
        ...
        Project project = builder.given_a_project();
        User user = builder.given_a_user();
        restUserControllerMockMvc.perform(post("/api/project/{project}/user/{user}/admin.json",
                project.getId(), user.getId())
                .contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk());
    ...
    public void delete_admin_from_project() throws Exception {
        ...
        builder.addUserToProject(project, user.getUsername(), READ);
        restUserControllerMockMvc.perform(delete("/api/project/{project}/user/{user}/admin.json",
                project.getId(), user.getId())
                .contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk());
    ...
    public void add_user_to_storage() throws Exception {
        ...
        Storage storage = builder.given_a_storage();
        User user = builder.given_a_user();
        restUserControllerMockMvc.perform(post("/api/storage/{storage}/user/{user}.json",
                storage.getId(), user.getId())
                .contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk());
    ...
    public void delete_user_from_storage() throws Exception {
        ...
        restUserControllerMockMvc.perform(delete("/api/storage/{storage}/user/{user}.json",
                storage.getId(), user.getId())
                .contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk());
    ...
    public void list_friends() throws Exception {
        ...
        restUserControllerMockMvc.perform(get("/api/user/{id}/friends.json",
                projectPrepresentative.getId())
    ...
    public void list_user_activity() throws Exception {
        ...
        restUserControllerMockMvc.perform(get("/api/project/{id}/usersActivity.json",
                project.getId()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.collection", hasSize(1)))
            .andExpect(jsonPath("$.collection[0].id").value(userOnline.getId()))
    ...
    void get_resume_activity() throws Exception {
        ...
        restUserControllerMockMvc.perform(get("/api/project/{id}/resumeActivity/{user}.json",
                project.getId(), userOnline.getId())
            )
            .andExpect(status().isOk())
    ...
    private MvcResult performDownload(String format, Project project, String type) throws Exception {
        return restUserControllerMockMvc.perform(get("/api/project/{project}/user/download",
                project.getId())
                .param("format", format))
            .andExpect(status().isOk())
